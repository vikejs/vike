...
import { getConfigEsbuildErrFormattedMsg } from '../plugins/importUserCode/v1-design/transpileAndLoadFile'
...
function isEquivalentOrSubset(err: unknown, errAlreadyLogged: unknown) {
  ...
  {
    const esbuildErrMsg1 = getConfigEsbuildErrFormattedMsg(err1)
    const esbuildErrMsg2 = getConfigEsbuildErrFormattedMsg(err2)
