  ...
  return [
    ...
    // This plugin appends `?extractAssets` to module IDs
    {
      ...
      async resolveId(source, importer, options) {
        ...
        // If the dependency is a VPS extension and has `configVps.extension[number].plusConfigsSrcDir`, then include its CSS
        if (
          configVps.extensions
            .filter(({ plusConfigsSrcDir }) => plusConfigsSrcDir !== null)
            .some(({ npmPackageName }) => {
              const check1 =
