...
test("page is rendered to HTML", async () => {
  ...
  expect(html).toMatch(partRegExp`{pageProps:{randomMessage:"${isHexNumber}"},apolloIntialState:{ROOT_QUERY`);
});
...
test("page is hydrated to DOM", async () => {
  ...
  expect(await page.textContent("body")).toMatch(partRegExp`Random message from server: ${isHexNumber}.`);
