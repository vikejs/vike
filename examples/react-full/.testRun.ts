...
import { run, page, test, expect, getServerUrl, fetchHtml, autoRetry, expectLog, sleep, partRegex } from '@brillout/test-e2e'
    ...
    if (uiFramework === 'react') {
      test('guard()', async () => {
        ...
        if (!isV1Design) {
          ...
          expectLog(partRegex`HTTP response ${/.*/} /hello/forbidden 401`, { filter: (log) => log.logSource === 'stderr' })
