...
function assertGlobalConfigLocation(
  ...
  sources.forEach((source) => {
    ...
    if (!isGlobalLocation(source.locationId, locationIdsAll)) {
      ...
      // When updating this error message => also update error message at http://vike.dev/warning/global-config
      const errMsg = `${errBeg} ${errMid}: ${errEnd} (http://vike.dev/warning/global-config).` as const
