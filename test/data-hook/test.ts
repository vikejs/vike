...
function testRun(cmd: 'npm run dev' | 'npm run preview') {
  ...
  test('HTML', async () => {
    const t = async (url: string, v1: string, v2: string, v3: string, v4: string, v5: string, v6: string) => {
      ...
      expect(html).toContain(`<p>global onBeforeRender() was called: <!-- -->${v1}</p>`)
      expect(html).toContain(`<p>global onBeforeRender() was called in env: <!-- -->${v2}</p>`)
      expect(html).toContain(`<p>global data() was called: <!-- -->${v3}</p>`)
      expect(html).toContain(`<p>global data() was called in env: <!-- -->${v4}</p>`)
      expect(html).toContain(`<p>per-page data() was called: <!-- -->${v5}</p>`)
      expect(html).toContain(`<p>per-page data() was called in env: <!-- -->${v6}</p>`)
    }
    await t('/', 'true', 'server', 'true', 'server', 'undefined', 'undefined')
    await t('/page-2', 'undefined', 'undefined', 'undefined', 'undefined', 'undefined', 'undefined')
    await t('/page-3', 'true', 'server', 'undefined', 'undefined', 'true', 'server')
    await t('/page-4', 'true', 'server', 'undefined', 'undefined', 'undefined', 'undefined')
  })
  ...
  test('Request pageContext JSON only if necessary', async () => {
    ...
    await testCounter()
    {
      // because neither onBeforeRender() nor data() are server-only
    ...
    {
      // because both onBeforeRender() and data() are server-only
    ...
    {
      // because both onBeforeRender() and data() are server-only, even though they are both null
      const done = expectPageContextJsonRequest(true)
