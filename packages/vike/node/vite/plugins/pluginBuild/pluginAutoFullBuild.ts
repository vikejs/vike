...
import { isViteClientBuild, isViteServerSide_onlySsrEnv } from '../../shared/isViteServerSide.js'
...
async function triggerFullBuild(config: ResolvedConfig, viteEnv: Environment, bundle: Record<string, unknown>) {
  ...
  if (isBuilderApp ? !isViteServerSide_onlySsrEnv(config, viteEnv) : !isViteClientBuild(config, viteEnv)) return
