        ...
        handler(code, id, options) {
          ...
          const replacements = Object.entries(envVarsAll)
            ...
            .map(([envName, envVal]) => {
              ...
              // Security check
              const securityCheckResult = checkEnvVarSecurity({
                envName,
                envStatement,
              ...
              if (!securityCheckResult.isValid) return
...
function checkEnvVarSecurity({
  ...
  if (isPrivate && isClientSide) {
    if (!new RegExp(envStatementRegExpStr).test(code)) return { isValid: false }
