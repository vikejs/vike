...
Don't show a warning when a page cannot be pre-rendered.
...
Alternatively, you can set `+prerender` to `false` for the pages that cannot be pre-rendered — this also suppresses the warning. See <Link href="#toggle" />.
...
>
> By using <Link text={<code>vite-plugin-vercel</code>} href="/vercel#vite-plugin-vercel" />, you can statically deploy your pre-rendered pages while using a production server for your non-pre-rendered pages.
