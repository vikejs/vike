...
The `globalContext` object is used for storing global information (i.e. information shared by mutliple pages).
...
Eagerly loaded runtime information about all your pages' <Link href="/config">configuration</Link> (e.g., all page routes).

<Prop name="config" />

Environment: server, client

Runtime information about your app's global <Link href="/config">configuration</Link>.
...
On edge environments, where there can be multiple server processes/workers, you can have multiple server-side `globalContext` object — one per process/worker.
