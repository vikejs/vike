...
> ```
>
> Likewise, both `<Head><something/></Head>` and `<Config Head={<something/>}/>` are also equivalent.

> Unlike other frameworks, Vike doesn't implement a `<Title>` component the title value must be a string — `<Title><b>Hello</b> World</Title>` wouldn't make sense.
