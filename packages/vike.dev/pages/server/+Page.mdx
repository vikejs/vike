...
> The `runtime` object is also available at <Link href="/pageContext#runtime">`pageContext.runtime`</Link>. Thus, the `pageContext` function above usually isn't need: you can simply access `pageContext.runtime.req.user` instead.
