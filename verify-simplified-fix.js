/**
 * This script verifies that our SIMPLIFIED fix for client-side guards works correctly.
 * The new approach: always call client-only guards, regardless of _hasPageContextFromServer.
 */

// Mock the simplified function
function mockGuardClientOnlyExists(pageContext) {
  // Simulate a client-only guard (e.g., +guard.client.js)
  return pageContext.hasClientOnlyGuard
}
...
function simulateGuardExecution(pageContext, isErrorPage = false) {
  ...
  // This is the SIMPLIFIED logic from our fix
  const shouldExecuteGuard = !isErrorPage && 
    (!pageContext._hasPageContextFromServer || mockGuardClientOnlyExists(pageContext))
...
// Test scenarios
console.log('=== Testing SIMPLIFIED Guard Execution Logic ===')
...
if (allPassed) {
  console.log('\n🎉 The SIMPLIFIED fix correctly handles client-side guards!')
  console.log('   - Client-only guards now execute on page reload (FIXED)')
  console.log('   - Client-only guards still execute on navigation')
  console.log('   - Server-only guards are not executed unnecessarily')
  console.log('   - Much simpler logic: just check if guard is client-only!')
} else {
