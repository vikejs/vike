...
By default `vite-plugin-ssr` does [Server Routing](/SR-vs-CR).
We can do [Client Routing](/SR-vs-CR) instead by using `useClientRouter()` instead of `getPage()`.

> In general, we recommend Server Routing over Client Routing, see [Server Routing VS Client Routing](/SR-vs-CR).
