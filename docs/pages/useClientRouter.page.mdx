...
const { hydrationPromise } = useClientRouter({
  ...
  // If we set `ensureHydration: true` then `vite-plugin-ssr` ensures that the first render is a
  // hydration (i.e. the first `render()` call is never interrupted even if a user clicks on a link)
