...
> Note that `pageContext` is completely discarded and loaded anew upon page navigation;
> that's why the context object is called `pageContext` and not `appContext`.
...
By default,
the static assets of a page `/some-url` are loaded as soon as the user hovers his mouse over a link `<a href="/some-url">`.
...
> Only static assets are prefetched; prefetching `pageContext` is work-in-progress, see [#246](https://github.com/brillout/vite-plugin-ssr/issues/246).
