...
const { hydrationPromise } = useClientRouter({
  ...
  // If `ensureHydration: true` then `vite-plugin-ssr` ensures that the first render is
  // a hydration. I.e., the first `render()` call is never interrupted — even if a user
  // clicks on a link. Default value: `false`.
