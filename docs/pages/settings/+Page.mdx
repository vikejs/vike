...
- [**`precompress`**](/precompress): Create `.br`/`.gz` files of static assets at build time.
...
- [**`rootAttributes`**](/rootAttributes): Add root element attributes (e.g. `<div id="root" role="none">`).
...
Settings that alias [Vite](https://vite.dev) settings (see also <Link href="/cli" />):
...
- [**`root`**](/root): Set the [project root directory](https://vite.dev/config/shared-options.html#root).
...
- [**`ai`**](/ai): Whether Vike checks your <Link href="/ai#skill">`vike/SKILL.md`</Link> is installed and up-to-date.
