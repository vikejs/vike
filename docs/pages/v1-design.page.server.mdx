...
// For Route Functions and Route Strings
export default route
```

> We explain down below why Route Functions cannot be defined in `+config.js`.
...
You can, if you want, define your entire app only by defining `+config.js` files:
 - One `/pages/+config.js` that defines your global configs.
 - One `/pages/some-page/+config.js` per page.
   > Your per-page `+config.js` files can completely override your global `/pages/+config.js`. (In essence, your global `+config.js` defines default values for all your pages.)

But you can also use following conveniences:
 ...
 1. `/renderer/+config.js` which is equivalent to `/pages/+config.js`.
    > `/renderer/+config.js` is only an (optional) convenience for having your renderer logic/files live outside of `/pages/**/*` (instead of cluttering your `/pages/` directory).
...
This works because VPS parses the import statements, such as `import Page` and `import onBeforeRender`, and extracts them outside of `+config.js` into separate internal virtual files. In case you're curious, see <Link href="/config-code-splitting" />.

<Note type='warning'>The import-based code splitting isn't implemented yet. In the meantime, use `+Page.js`, `+onBeforeRender.js`, ...</Note>

> The `+` convention is [controversial](https://github.com/brillout/vite-plugin-ssr/issues/578#issuecomment-1472005734). While we think that `+` files are "ugly" at first, we ask you to trust us: we believe you'll eventually end up liking the `+` convention as much as we did.
