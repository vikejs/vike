...
This works because VPS parses the import statements, such as `import Page` and `import onBeforeRender`, and extracts the imports outside of `+config.js` into separate internal virtual files. In case you're curious, see <Link href="/config-code-splitting" />.
