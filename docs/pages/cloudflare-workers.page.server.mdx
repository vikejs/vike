...
> The example set `wrangler.toml#node_compat` to `true` which makes wrangler show a warning:
> ```
> [WARNING] Enabling node.js compatibility mode for built-ins and globals. This
> is experimental and has serious tradeoffs.
> ````
> But we can safely ignore it (the only Node.js shims used are all robust).
>
