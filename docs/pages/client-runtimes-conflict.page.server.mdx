...
Then the common causes are:
 - You configured [Rollup's code splitting](https://vitejs.dev/guide/build.html#chunking-strategy) in a way that makes both runtimes being included in a same bundle.
   ```js
   // vite.config.js
   ...
   // striclty needed for that page. (Initial page load speed is usually more important than
   ...
   ```
 - You're importing a utility, such as `prefetch()` or `navigate()`, that is only available to <Link text="Client Routing" href="/client-routing" />.
   ```js
   ...
   > ```
   > If you don't get this warning, then the issue is most likely related to your Rollup configuration.
