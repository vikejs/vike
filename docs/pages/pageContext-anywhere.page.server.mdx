...
> If we use <Link href="/extensions#ui-framework">`vike-react`</Link>, then we can use its <Link href="/usePageContext">`usePageContext()`</Link> component hook.

We can use [React Context](https://reactjs.org/docs/context.html)
to make `pageContext` accessible from any React component.

Example:
 - <RepoLink path='/boilerplates/boilerplate-react/renderer/usePageContext.jsx' /> (component hook <code>usePageContext()</code> implementation)
...
> If we use <Link href="/extensions#ui-framework">`vike-vue`</Link>, then we can use its <Link href="/usePageContext">`usePageContext()`</Link> component hook.
...
We can also implement a `usePageContext()` component hook:
