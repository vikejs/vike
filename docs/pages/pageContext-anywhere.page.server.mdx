...
> If we use <Link href="/extensions#ui-frameworks">`vike-react`</Link>, then we can use its <Link href="/usePageContext">`usePageContext`</Link> hook.
...
> If we use <Link href="/extensions#ui-frameworks">`vike-vue`</Link>, then we can use its <Link href="/usePageContext">`usePageContext`</Link> hook.
