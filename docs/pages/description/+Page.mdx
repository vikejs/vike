...
On the server, upon rendering the HTML of the first page, the `description` setting adds the `<meta name="description">` and `<meta property="og:description">` tags to the [`<head>` tag](https://developer.mozilla.org/en-US/docs/Learn/HTML/Introduction_to_HTML/The_head_metadata_in_HTML).

On the client, upon <Link href="/client-routing">client-side page navigation</Link>, the `description` setting dynamically updates these tags by mutating the DOM.
