...
<StaticSsrIntro provider={<a href="https://vercel.com">Vercel</a>} />
...
To deploy to [Vercel Edge](https://vercel.com/docs/functions/edge-functions/edge-runtime):
...
Example:
 - <Example timestamp="2026.04" repo="mrdsx/vike-with-vercel" />
...
Using `+server` you can use the `entry` option ([+server Custom server entrypoint](https://vike.dev/vercel#custom-server-entrypoint)) to export the server and the optional exports.
...
I you are using additional exports in `+server`, for example `getDocuments`, you can also import them in the Vercel's API, for example with CRON jobs:

```js
// api/cron.js

import { getDocuments } from '../dist/server/index.mjs'
