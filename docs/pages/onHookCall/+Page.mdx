...
> You must call `hook.call()` synchronously (before any `await`). This is because sync hooks cannot be wrapped asynchronously while preserving synchronous execution.
>
> ```ts
