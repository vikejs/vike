...
<ConfigSpec
  env="server (and/or client)"
  cumulative
  global
>
```ts ts-only hide-menu
type OnHookCall = (hook: Hook, pageContext: null | PageContextServer) => void
type Hook = {
  name: string
  filePath: string
...
> To execute it on the client, define `+onHookCall.client.js` or `+onHookCall.shared.js` — see <Link href="/file-env" />.
