...
If we don't use pre-rendering, then we need a Node.js production server (or a Node.js-like environement such as <Link href="/cloudflare-workers" text="Cloudflare Workers" /> or <Link href="/vercel" text="Vercel" />) in order to be able to dynamically render our pages' HTML at request-time.
