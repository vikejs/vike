...
You can keep using `<a href="/some-url">` links: Vike's client-side router automatically intercepts clicks on `<a>` elements.

You can skip Vike's client-side router from intercepting individual `<a>` links by adding the `data-vike="false"` or `rel="external"` attribute, e.g. `<a data-vike="false" href="/some/url">Vike's client router won't intercept me</a>`.

For example, to enable smooth scrolling for some links:

```tsx
function LinkSmooth({ href, ...props }: { href: string; children: React.ReactNode }) {
  return <a data-vike={false} onClick={onClick} {...props} />
