...
You can skip Vike's client-side router from intercepting individual `<a>` links by adding the `data-vike="false"` attribute, e.g. `<a data-vike="false" href="/some/url">Vike's client router won't intercept me</a>`.
