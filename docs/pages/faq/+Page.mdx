...
> There aren't any rules — you can post whatever you want. That said, questions sometimes go unanswered on Discord, so we recommend checking out the `#help-guide` channel to increase your chances of getting help. Also, consider giving back by helping others!
...
> Only asks questions related to Vike and follow [these rules](https://brillout.github.io/rules/).
...
Vike supports all (common) use cases:
 ...
 - <Link href="/pre-rendering#should-i-pre-render" doNotInferSectionTitle>Without backend</Link>, with <Link href="/server">any JavaScript backend</Link> (Express.js, Hono, Fastify, Adonis, Nest, Deno, Bun, etc.), or <Link href="/integration#non-javascript-backend">with other backends</Link> (Java, Ruby on Rails, PHP, etc.).
 - <Link href="/tools#data-fetching">Any data fetching tool</Link> (REST, RPC, GraphQL, etc.).
 - Optional <Link href="/ssr">Server-Side Rendering (SSR)</Link>, optional <Link href="/pre-rendering">pre-rendering (SSG)</Link>, and optional <Link href="/vercel#isr">Incremental Static Regeneration (ISR)</Link>.
 - Optional <Link href="/stream">HTML streaming</Link> (with <Link href="/streaming#progressive-rendering">Progressive Rendering</Link>).
 - Deploy anywhere (VPS, AWS, GCP, Cloudflare, Vercel, etc.).
