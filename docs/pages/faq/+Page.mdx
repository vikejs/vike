...
Vike supports all (common) use cases:
 ...
 - <Link href="/pre-rendering#should-i-pre-render">Without backend</Link>, with <Link href="/server">any JavaScript backend</Link> (Express.js, Hono, Fastify, Adonis, Nest, Deno, Bun, etc.), or <Link href="/integration#non-javascript-backend">with other backends</Link> (Java, Ruby on Rails, PHP, etc.).
