...
Vike supports all (common) use cases:
 ...
 - <Link href="/pre-rendering#should-i-pre-render">Without backend</Link>, with <Link href="/server-integration">any JavaScript backend</Link> (Express.js, Hono, Fastify, Adonis, Nest, Deno, Bun, etc.), or <Link href="/server-integration#non-javascript-backend" doNotInferSectionTitle>with other backends</Link> (Java, Ruby on Rails, PHP, etc.).
