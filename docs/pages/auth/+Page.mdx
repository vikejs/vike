...
export async function onCreatePageContext(pageContext: PageContextServer) {
  ...
  user = await someAuthApi.getUser(req.headers)
...
> If you use <Link href="/vike-server">`vike-server`</Link>, you can access the request object at `pageContext.runtime.req.user` (see <Link href="/pageContext#runtime">`pageContext.runtime`</Link>).
>
