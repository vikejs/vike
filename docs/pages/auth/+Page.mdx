...
export async function onCreatePageContext(pageContext: PageContextServer) {
  // The request object is available at pageContext.req
  ...
  // @docpress-uncomment const user = await someAuthApi.getUser(req.headers)
...
> See also: <Link href="/pageContext#req">API > `pageContext.req`</Link>
...
> If you use <Link href="/server">`+server.js`</Link>, you can access the request object at <Link href="/pageContext#req">`pageContext.req`</Link> (an alias of <Link href="/server#pagecontext-runtime">`pageContext.runtime.req`</Link>).
>
> If you don't use `+server.js`, you can define a custom property `pageContext.req` at <Link href="/renderPage">`renderPage()`</Link>:
> ```ts
> // server/index.ts
>
