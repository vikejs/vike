...
`passToClient` is usually used together with the `addPageContext()` hook to fetch data:
data is fetched in `async addPageContext()` and then made available to the browser with `passToClient`, see [Data Fetching](/data-fetching).

Serialization is done with [`devalue`](https://github.com/Rich-Harris/devalue).
