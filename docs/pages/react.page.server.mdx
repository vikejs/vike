import { UseIntegrationPackageHint, Example } from '../components'
...
Instead of using a Vike extension, you can manually integrate React yourself, giving you full control over the integration of not only React itself but also React libraries.

Official examples:
 - <Link href="/examples/react" /> - Basic React integration, using <Link href="/server-routing">Server Routing</Link>.
 - <Link href="/examples/react-full" /> - Full-fledged React integration, using <Link href="/client-routing">Client Routing</Link> and HTML streaming.
 - <Link href="/examples/graphql-apollo-react" /> - React integration with GraphQL Apollo.
 - <Link href="/examples/render-modes" /> - React integration supporting all <Link href="/render-modes">render modes</Link> (SSR, SPA, and HTML-only).

> For more official examples, see [GitHub > `vikejs/vike` > `examples/`](https://github.com/vikejs/vike/tree/main/examples).

Community examples:
 - <Example timestamp="2023.12" repo="luisfloat/vike-island-example" description={<><a href="https://jasonformat.com/islands-architecture/">Islands Architecture</a> implementation, mixing React and Vue.</>} />
