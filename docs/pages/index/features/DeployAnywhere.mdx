...
Also, `vite-plugin-ssr` has been designed with first-class Cloudflare Workers support in mind, for example:
 - For development, you can use Vite with an Express.js dev server for lightning fast HMR & dev start, and, for production, you can bundle your worker code with Cloudflare Workers's `wrangler` bundler.
 - `vite-plugin-ssr` works with Cloudflare Workers's HTML Streaming support.
