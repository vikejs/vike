...
Thanks to `vite-plugin-ssr`'s lean architecture, your app is much less limited by Cloudflare Workers' 1MB bundle size limit.

In general, `vite-plugin-ssr` has been designed with Cloudflare Workers in mind, for example:
 - Dev with Vite + Express.js, bundle & deploy with Cloudflare Workers
 - Support for HTML Streaming with Cloudflare Workers
