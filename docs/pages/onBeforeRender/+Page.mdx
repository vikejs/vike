...
function data() {
  // @docpress-uncomment const someValue = /* ... */
...
function onBeforeRender() {
  // @docpress-uncomment const someValue1 = /* ... */
  // @docpress-uncomment const someValue2 = /* ... */
...
By default, like `data()`, the `onBeforeRender()` hook always runs on the server-side.

By using <Link href="/file-env">`.shared.js` (or `.client.js`)</Link> you tell Vike to also (or only) load and execute `onBeforeRender()` on the client-side, see:
- <Link href="/data#client-js" doNotInferSectionTitle />
- <Link href="/data#shared-js" doNotInferSectionTitle />
