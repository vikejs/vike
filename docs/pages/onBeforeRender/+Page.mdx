...
Like `data()`, the `onBeforeRender()` hook always runs on the server-side by default. But you can use the <Link href="/file-env">file extension `.shared.js`</Link> to make Vike run `onBeforeRender()` on the client-side, see <Link href="/data#environment" />.
