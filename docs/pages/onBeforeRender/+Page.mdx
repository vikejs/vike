import { HookTypeScriptHints, UiFrameworkVikeExtensionNames } from '../../components'
...
Like `data()`, the `onBeforeRender()` hook always runs on the server-side but you can use the <Link href="/file-env">file extension `.shared.js`</Link> to make Vike run `onBeforeRender()` on the client-side, see <Link href="/data#client-side" />.
...
> If you use a custom renderer instead of <UiFrameworkVikeExtensionNames />, then you can modify your `onRenderHtml()`/`onRenderClient()` hooks instead of doing the following.
