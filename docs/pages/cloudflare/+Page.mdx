...
## `vike-photon` with `@photonjs/cloudflare`

<Link href="/vike-photon" /> enables zero-configuration support for most Cloudflare features, including [static assets deployment](https://developers.cloudflare.com/workers/static-assets/get-started/#deploy-a-static-site) or [SSR](https://developers.cloudflare.com/workers/static-assets/get-started/#deploy-a-full-stack-application).
...
> Using a server is optional with <Link href="/vike-photon" />, even when SSR is enabled
...
See <Link href="/vike-photon#get-started" /> for instructions concerning `server/index.ts`

## Deploy

To deploy an app to [Cloudflare Workers](https://developers.cloudflare.com/workers/), run the `deploy` script:
