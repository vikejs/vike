...
If you <Link href="/pre-rendering">pre-render</Link> all your pages then you don't need a server — simply add the `vike-cloudflare` extension:
...
<Tabs>
  ...
  <TabPanel>
    ```shell
    npm i -D wrangler vike-cloudflare
    ```
  </TabPanel>
  <TabPanel>
    ```shell
    pnpm add -D wrangler vike-cloudflare
    ```
  </TabPanel>
  <TabPanel>
    ```shell
    yarn add -D wrangler vike-cloudflare
    ```
  </TabPanel>
</Tabs>
...
```json
{
  "scripts": {
    "dev": "vike dev",
    "build": "vike build",
    "preview": "npm run build && wrangler pages dev", // [!code ++]
    "deploy": "npm run build && wrangler pages deploy" // [!code ++]
  }
}
```
...
If you use <Link href="/ssr">SSR</Link>, you must have a server and you can add `vike-cloudflare` like this:
...
With <Link href="/onCreateGlobalContext">`+onCreateGlobalContext.server`</Link>, you can make Cloudflare's bindings (the `env` object) available anywhere via <Link href="/globalContext#custom">`globalContext.cloudflare.env`</Link>.
...
> For an example of how to access [Cloudflare D1](https://developers.cloudflare.com/d1/) during development, see `npm create vike@latest --react --hono --drizzle --cloudflare` or go to <Link href="/new">vike.dev/new</Link> and `Cloudflare` and an ORM.
