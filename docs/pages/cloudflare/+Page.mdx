...
See:
 - <Link href="#with-server" /> if you use <Link href="/ssr">SSR</Link> (SSR requires a server).
...
> `vike-cloudflare` currently supports <Link href="/hono">Hono</Link> and <Link href="/hattip">HatTip</Link> — more servers are coming soon.
...

{
  ...
  // Only needed if your app (or one of your libraries) uses a Node.js API
