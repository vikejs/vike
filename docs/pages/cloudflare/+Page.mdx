...
> You can export Durable Objects inside `+server.js`, see section below <Link href="#durable-objects" />.
...
You can export [Durable Objects](https://developers.cloudflare.com/durable-objects) inside your <Link href="/server">`+server.js`</Link> file:
...
export class MyDurableObject extends DurableObject { // [!code ++]
  // ... // [!code ++]
} // [!code ++]
```

> If you use Vike's built-in server (<Link href="/server#server-true">`+server: true`</Link>), then define the following `+server.js` instead:
>
> ```ts
> // +server.ts
>
