...
Instead of defining `+config.h.js` we could define `+config.js`:
...
But then we loose all the IntelliSense niceties. That's why we recommend to use `+config.h.js` instead of `+config.js`.
