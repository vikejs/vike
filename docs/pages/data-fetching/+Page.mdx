...
async function data(pageContext: PageContextServer) {
  ...
  let movie: MovieDetails = await response.json()
  ...
  /* With an ORM:
  const movie = await Movie.findAll({ select: ['title', 'release_date'] }) */
  /* With SQL:
  const movie = await sql.run('SELECT { title, release_date } FROM movies;') */
