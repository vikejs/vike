...
The usual recommendation is:
 - For fetching the page's initial data, use Vike's `+data` hook, see <Link href="#page-data-with-data" />.
 - For data mutations and subsequent data (e.g. pagination), use a tool such as Telefunc, tRPC, or TanStack Query, see <Link href="#data-mutation-subsequent-data" />.
...
async function data(pageContext: PageContextServer) {
  ...
  // By default, +data runs only on the server-side, we can therefore use ORM/SQL queries.
  /* With an ORM:
