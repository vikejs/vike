...
To fetch and mutate data, the default recommendation is to use `data()` along with a data-fetching tool.

## `data()`

To fetch the initial data of a page, you can use the <Link href="/data">Vike hook `data()`</Link> together with the <Link href="/useData">component hook `useData()`</Link>.
