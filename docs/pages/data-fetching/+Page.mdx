...
The default recommendation is to use `data()` for fetching initial data, and a data-fetching tool for data mutation and subsequent data (such as pagination data).
...
The `data()` hook can only be used for fetching the initial data of the page. For other use cases, such as data mutation and pagination data, use a data-fetching tool.
...
> We are currently working on <Link href="/extensions">Vike extensions</Link>, such as [`vike-react-query`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-query) and `vike-vue-telefunc`, that enable you to use your favorite data-fetching tool also for fetching initial data, in a seamless and zero-config fashion.


## Data mutation & subsequent data

For data mutation and subsequent data fetching (such as pagination data), use a data-fetching tool.
...
For a reliable handling of complex state logic, you can use state management stores. See following <Link href="/extensions">Vike Extensions</Link> for automatic integration:
...
You can also manually integrate state management stores (Redux/Zustand/...) yourself, giving you complete control over the integration, see <Link href="/data-stores" />.
