...
You can add such data <Link href="/globalContext#custom">to `globalContext`</Link> and/or <Link href="/pageContext#custom">to `pageContext`</Link>.

The `globalContext` object is accessible anywhere, while `pageContext` is accessible from any <Link href="/hooks">Vike hook</Link> or UI component. See:
