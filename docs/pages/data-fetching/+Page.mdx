...
The default recommendation is to use `data()` for fetching initial data, and a data-fetching tool for data mutations and subsequent data (such as pagination data).
...
The `data()` hook can only be used for fetching the initial data of the page. For other use cases, such as data mutations and pagination data, use a data-fetching tool.
...
For complete control over integration, instead of using an extension, you can manually integrate a store yourself. See <Link href="/data-stores" />.
