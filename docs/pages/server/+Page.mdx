...
The `serve()` helper can further be configured with the following options:
```js
serve(app, {
  ...
  onServer(server) {
    ...
    //   node:  Server ('node:http') by default,
    //          could be an HTTPS or HTTP2 server if `createServer` is provided (see below)
    //   deno:  return of Deno.Serve (experimental support)
    //   bun:   return of Bun.Serve  (experimental support)

    // As no server is running under serverless environments like Cloudflare or Vercel Edge,
    // `server` is `undefined` in those runtimes.
  },

  // The following options are available when running on Node only!
  ...
  createServer,
  // Options forwarded to `createServer`
  serverOptions,
})
```
