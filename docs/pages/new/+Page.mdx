...
> Apps scaffolded by Bati use <UiFrameworkVikeExtension name links/>.

## Without `vike-{react,vue,solid}`

You can also create a new Vike app that doesn't use <UiFrameworkVikeExtension name links/>, which means you control the React/Vue/Solid integration.

<Warning>
Do this only if you are an expert user and only if you have a strong & clear reason for not using <UiFrameworkVikeExtension name />.
</Warning>
