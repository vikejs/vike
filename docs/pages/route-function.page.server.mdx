...
But this isn't recommended: `pageContext.routeParams` is supposed to hold only a minimal amount of information. Instead, we recommend to implement complex logic in <Link text={<code>onBeforeRender()</code>} href="/onBeforeRender" />, <Link text={<code>guard()</code>} href="/guard" />, or in a <Link text="custom hook" href="/meta" />.
