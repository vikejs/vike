...
But this isn't recommended: `pageContext.routeParams` is supposed to hold only a minimal amount of information. Instead, we should prefer implementing complex logic in <Link text={<code>onBeforeRender()</code>} href="/onBeforeRender" /> and/or in <Link text="custom exports/hooks" href="/exports" />.
