...
The motivation for async route functions are often redirections, but using <Link href="/RenderErrorPage" text={<code>throw RenderErrorPage()</code>}></Link> is usually a better approach.
