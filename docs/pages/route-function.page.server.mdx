...
The motivation for an async Route Function is often redirection and/or protecting a private page; instead use <Link href="/RenderErrorPage" text={<code>throw RenderErrorPage()</code>}></Link> with an async <Link text={<code>guard()</code>} href="/guard" /> or <Link text={<code>onBeforeRender()</code>} href="/onBeforeRender" /> hook.
