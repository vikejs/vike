...
Also, if our page is often viewed on mobile devices, then SSR is important in order to quickly dispaly content to our users before the entire JavaScript loads & executes (which is usually very slow on mobile).
