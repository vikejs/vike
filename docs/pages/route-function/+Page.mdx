...
The motivation for having an asynchronous Route Function is usually to implement redirection and/or protecting a private
page. Instead, use <Link href="/render" text={<code>throw render()</code>} /> or<Link href="/redirect" text={<code>throw redirect()</code>} />
with an async <Link text={<code>guard()</code>} href="/guard" />, <Link text={<code>data()</code>} href="/data" /> or
<Link text={<code>onBeforeRender()</code>} href="/onBeforeRender" /> hook.
