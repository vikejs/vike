...
import { JustAMiddleware, VikeExtensionsUiFrameworksNamesEnglish } from '../components'
...
Vike extensions such as <VikeExtensionsUiFrameworksNamesEnglish /> integrate UI frameworks so that you don't have to write the integration.

But, if you want, instead of using <Link href="/extensions">Vike extensions</Link> you can use the hooks `onRenderHtml()` and `onRenderClient()` for complete control over how pages are rendered.
