...
- Make sure the page doesn't use any server-only `data()`, `onCreatePageContext()`, or `onBeforeRender()` hook.
  ...
  > Consider using `.ssr.js` instead of `.server.js`, see <Link href="/file-env#ssr-js" doNotInferSectionTitle />.
- If you have a <Link href="/integration#server-manual-integration">custom server integration</Link> (i.e. you don't use <Link href="/server">`+server.js`</Link>), make sure the `pageContextInit` you pass to <Link href="/renderPage">`renderPage()`</Link> doesn't contain a property included in <Link href="/passToClient">`passToClient`</Link>.
  ```ts
