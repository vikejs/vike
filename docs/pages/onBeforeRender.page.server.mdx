...
> This is an advanced, low-level hook. For a simpler, higher-level hook, see <Link href="/data" />.
...
By default, `onBeforeRender()` always runs on the server-side. The fetched data can be made available on the client-side
