...
A simple way to define metadata is to define a `metadata.js` file that contains <Link href="#metadata:~:text=frontmatter-,Global%20metadata,-What">global metadata</Link>.
...
You can use <Link href="/meta">`meta`</Link> to create custom settings for defining <Link href="#metadata:~:text=.-,Local%20metadata,-What">local metadata</Link>.
