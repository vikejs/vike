...
import { GithubLink, ViteLazyTranspilingContradiction } from '../../components'
...
The preferred technique depends on whether you want to define global or page metadata.
...
> **What is page metadata?**  For example, you want to show detailed information below a blog post, such as the author's name. This metadata is shown only for that page.

For page metadata, we recommend:
...
You can use <Link href="/meta">`meta`</Link> to create a custom setting for defining <Link href="#page-metadata">page metadata</Link>.
...
You can use such frontmatter to define <Link href="#page-metadata">page metadata</Link>.
...
- <GithubLink repo="blankeos/gittydocs" /> - Documentation builder.
