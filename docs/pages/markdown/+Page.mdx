...
> Unlike in the example <Link href="#with-a-custom-setting">below</Link>, you cannot define `+metadata` inside `+Page.mdx` files because each `+Page.mdx` is <Link href="/lazy-transpiling">loaded lazily</Link> when rendering the page. Consequently, the content of `+Page.mdx` isn't available to other pages — it's available only locally.
...
> See also: <Link href="/onCreateGlobalContext" />.
