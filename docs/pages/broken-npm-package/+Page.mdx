...
> The reason why an npm package can contain invalid code is because the transpilation process of some frameworks, most notably Next.js, isn't strict and tolerates erroneous code: thus an npm package can work with Next.js while not work in other JavaScript environments. Learn more at <Link href="#why" />.
>
> The situation is going to improve over time as Vite-powered frameworks gain popularity.
...
> Feel free to <Link href="/faq#can-i-reach-out-for-help">reach out for help</Link> if the workarounds don't work.
...
> The issue is most widepread in the React ecosystem because of Next.js's prevalence, but this is going to less and less of an issue as Vite-powered React frameworks gain popularity.
...
```js
import theDefaultExport from 'some-library' // [!code --]
import lib from 'some-library'; // [!code ++]
const theDefaultExport = lib.default ?? lib // [!code ++]
```
...
Use the component trace to find out which component is `undefined` / an `object`. You'll likely discover that the React component is imported, while the import value is `undefined` / an `object` (instead of a React component) because of CJS/ESM interoperability quirks.
