...
> The reason why some npm packages contain invalid code is because the transpilation process of some frameworks, most notably Next.js, isn't strict and tolerates erroneous code: thus an npm package can work with Next.js while not work in other JavaScript environments. You can learn more at <Link href="#why" />.
>
> The situation is going to improve over time as Vite-powered frameworks gain popularity.

## Workaround

You can workaround problematic npm packages by using either:
 - `ssr.noExternal` or
 - `vite-plugin-cjs-interop`.

> The workarounds usually work. But if you struggle working around a broken npm package then feel free to <Link href="/faq#can-i-reach-out-for-help">reach out for help</Link>.
...
See also: [Vite > `ssr.noExternal`](https://vitejs.dev/config/ssr-options.html#ssr-noexternal).
...
Upon certain errors, we recommend trying [vite-plugin-cjs-interop](https://github.com/cyco130/vite-plugin-cjs-interop) first before trying `ssr.noExternal`.
...
export default {
  plugins: [
    cjsInterop({
      dependencies: [
        // Add problematic npm package here
        "some-package/**", // Use /** to apply the interop patch to all imports
      ]
    })
  ]
}
...
Errors for which we recommend trying `vite-plugin-cjs-interop` first before trying `ssr.noExternal`:
