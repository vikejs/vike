...
Hooks with `.ssr.js` behave like `.server.js` but **run only during SSR** (initial page load on the server) — not during client-side navigation (<Link href="/pageContext.json">`pageContext.json` requests</Link>).
