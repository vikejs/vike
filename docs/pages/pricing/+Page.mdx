...
Vike's free tier is determined by what we call the *double three cut-off*:

- **`>=3` regular committers for the past `>=3` months.**

Consequently:

- Vike is free for small projects. (Maintained by `<=2` regular committers.)
- Vike is free for large projects in their infancy. (With 3 months free trial from the moment the projects starts to have `>=3` regular committers.)
- Vike is free for finished projects. (They can be maintained for free with up to two full-time developers.)

If you are within the free tier then you don't need any license key and you can use Vike without any restrictions, just like any other open source tool. (See also <Link href="#how-does-it-work" />)

> Small contributors, such as bots or a designer making occasional commits, aren't counted as "regular committers".

- Vike is free for apps maintained by one or two maintainers in the past three months

An app is considered "large" if it has three or more developers who have committed regularly over the past three months.

If you have 2 or less 
You don't need any license key and you can use Vike without any restrictions, just like any other open source tool.

Not only is Vike 100% free

- **Regular committer**: developer who made significant commits over the past three months.


### Recommendation

The table above are only recommendations: as a company, you can choose any amount you want to pay. We trust companies to make a fair choice.

> If you didn't incorporate yet, or you just want to experiment, feel free to apply for a free license key.

you believe your organization should be part, then feel free to reach out  or you have a special financial situation

If you're , then you'll a license key but you can apply for a free one — we'll give you a free license key.

> **Don't hesitate to apply for a free license key** if this recommended sponsoring amount isn't affordable for you. Keeping Vike accessible for everyone is a fundamental goal of the Open Source Pricing.
...
- **Large organization**: >70 employees. (Also counting the employees of all parent companies.)
...
- **Large use case**: Vike is used for developing a **project that plays a significant role** to your company. For example an app that many users interact with, or using Vike for building your own framework.
...
The ovarraching goal here is to have Vike be 100% gratis for small use cases while staying affordable for everyone else.
