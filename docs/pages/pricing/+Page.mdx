...
You can apply for a free license key if your app is considered large but your company isn't large enough to pay for a license key.
...
For example, in the following, it only applies to `my-app/` and `my-other-app/`.
...
> Technically, it applies to domain names. A license key contains a list of domain names `['example.org', 'example.com', ...]` for which the pesky toaster won't be shown. You specify that list of domain names when you purchase a license key.
...
For larger projects, Vike shows a pesky toaster `Get your license key at ...` that you can remove by purchasing a license key. (You can apply for a free license key if you have limited resources.) You install the key by adding it to your `+config.js` file, or by setting the `VIKE_KEY` environment variable.

> See also: <Link doNotInferSectionTitle href="#when-is-an-app-considered-small-or-large" />
