...
As a larger team, you will see a pesky toaster `Get your license key at ...` in development (it's never shown in production). You can remove it by purchasing a license key and adding it to `+config.js` or by setting the `VIKE_KEY` environment variable.
