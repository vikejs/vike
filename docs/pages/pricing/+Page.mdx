...
Vike is still free as in "free speech" (not as in free beer). See the explanation [by the Free Software Foundation](https://www.gnu.org/philosophy/free-sw.en.html) and [by Wikipedia](https://en.wikipedia.org/wiki/Free_software).

Our passion for Libre Software was the reason we invented the Open Source Pricing. We believe <Link doNotInferSectionTitle href="#why-not-another-business-model" >other business models are fundamentally flawed</Link>.

Code remains not only 100% open source but also 100% accessible: we believe money (and the lack thereof) should never be a showstopper for using code.
