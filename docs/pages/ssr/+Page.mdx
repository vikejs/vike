...
Alternatively, instead of using the `+ssr` setting, you can use <Link href="/ClientOnly">`<ClientOnly>`</Link> to render and load some components only on the client-side, while rendering the rest of the page with SSR.
