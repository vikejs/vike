...
> Instead of manually integrating render modes yourself, you can use <Link text={<><code>vike-*</code> packages</>} href="/vike-packages" /> and [Bati](https://batijs.github.io/): `vike-*` packages include an `ssr: boolean` config and Bati helps you scaffold projects. Bati and `vike-*` packages are going to get out of beta soon.
...
To achieve that:
 1. We set the `Page` [meta config](/meta#modify-existing-configurations) to `client-only` instead of `server-and-client`.
 2. We adapt our `onRenderHtml()` and `onRenderClient()` hooks.


**1. `Page` meta config**

By setting the `Page` meta config to `client-only` we ensure that `+Page.js` is loaded only in the browser.
...
export default {
  meta: {
    Page: {
      env: 'client-only' // SPA for all pages
    }
  }
}
```
...
This is the key difference between SPA and SSR: in SPA `div#root` is empty, whereas with SSR `div#root` contains our
Page's root component `pageContext.Page` rendered to HTML.
...
async function onRenderHtml(pageContext) {
  const { title, description } = pageContext.exports
...
> If we set the `Page` meta config to `client-only` instead of `server-and-client` in `config.h.js`, then
> `pageContext.Page` is only defined in the browser.
