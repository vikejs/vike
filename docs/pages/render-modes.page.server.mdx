...
If we only have SPA pages, then we adapt our `render()` hooks like the following.
...
This means that we define both `about.page.js` (defining metadata) and `about.page.client.js` (defining our page's root component `pageContext.Page`).
