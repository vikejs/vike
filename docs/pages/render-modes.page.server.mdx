...
**2. `onRender*()` hooks (SPA only)**

If we only have SPA pages, then we adapt our `onRender*()` hooks like the following.
...
**2. `onRender*()` hooks (SPA + SSR)**

If we have both SPA and SSR pages, then we adapt our `onRender*()` hooks like this:
