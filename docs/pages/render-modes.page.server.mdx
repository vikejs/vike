...
> Note that the server-side `render()` hook is still called in order to generate the static HTML of our SPA page.
> For production, if we only have SPA pages, then we may want to <Link text="pre-render" href="/pre-rendering" /> the static HTML of our SPA pages at build-time.
