...
```js
// /pages/rocket/starship/+title.jsx
// Environment: server, client
...
> As shown [here](https://gist.github.com/brillout/e7fe603bd44c9ec1be7e98f3ae28636a) you can define the logic, of using `data.title` to set the page's title, once and apply to all your pages.
...
// Setting the title value to a function instead of a string,
// in order to be able to use component hooks such as usePageContext()
function title() {
  const pageContext = usePageContext()
