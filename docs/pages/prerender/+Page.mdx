...
Type: `boolean`.  
Default: `true` if app is fully-prerendered.  

Whether <Link href="/redirects">`+redirects`</Link> should be pre-rendered to HTML documents that redirect the user:

```html
<!-- HTML redirecting user to https://example.org/path (real HTML has styles and meta data) -->
<html>
<head>
...
<body>
  <!-- Fallback (only shown if the meta tag above doesn't work) -->
  <h1>Redirect <a href="https://example.org/path">https://example.org/path</a></h1>
