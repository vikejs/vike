...
type GlobalSetting = PageSetting | {
  ...
  keepDistServer?: boolean // Default: `false`
}
...
type PrerenderConfigLocal = PageSetting | (() => PageSetting | Promise<PageSetting>)
```
</ConfigSpec>
...
> Note that:
> - Global settings (`GlobalSetting` above) must be defined at a global location.
> - Page settings (`PageSetting` above) can be defined anywhere (locally as well as globally).
>
> See also: <Link href="/config#inheritance" />.
>
> ```ts
> // 👉 Local config (i.e. doesn't apply to all pages)
...
>
> ```ts
> // 👉 Global config (i.e. applies to all pages)
> // pages/+config.ts
>
> import type { Config } from 'vike/types'
>
