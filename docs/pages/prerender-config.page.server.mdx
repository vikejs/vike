...
When running `$ vite build`, Vike automatically triggers <Link href="/pre-rendering">pre-rendering</Link>. (If you <Link text="enabled pre-rendering" href="/pre-rendering#how-to-pre-render" />.)

You can disable the automatic triggering of the pre-rendering step:
...
You can then manually trigger the pre-rendering process using:
