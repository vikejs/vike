...
> As explained in <Link href="/pre-rendering" />, if we don't pre-render all pages, then we need a production server. By using, [`vite-plugin-vercel`](https://github.com/magne4000/vite-plugin-vercel#usage-with-vite-plugin-ssr), we can still statically deploy our pre-rendered pages while using a production server for non-pre-rendered pages.
