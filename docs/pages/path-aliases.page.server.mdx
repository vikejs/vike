...
Your path aliases need to start with a special character, so that your aliased imports can be distinguished from npm package imports.
We recommend following the [`#` prefix convention](https://nodejs.org/api/packages.html#subpath-imports:~:text=must%20always%20start%20with%20%23%20to%20ensure%20they%20are%20disambiguated%20from%20external%20package%20specifiers).
