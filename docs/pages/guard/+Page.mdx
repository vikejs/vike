...
The `guard()` hook is the first hook called after the routing is evaluated. It's always called before the <Link href="/data">`data()` hook</Link>. See <Link href="/hooks#order" />.
...
The `guard()` hook is called in the same environment as <Link href="/data">`data()`</Link>. In other words, it's always called on the server-side unless you <Link href="/data#client-side">configure `data()` to run on the client-side</Link>.
