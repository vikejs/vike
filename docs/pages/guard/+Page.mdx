...
By default, the `guard()` hook is:
- Called in the same environment as <Link href="/data">`data()`</Link>. In other words, it's always called on the server-side unless you <Link href="/data#environment">configure `data()` to run on the client-side</Link>.
- Loaded in both the client and server (even if it only executes on the server-side).

> By default, your `guard()` hook (and the entire `+guard.js` file) is always loaded in 
...
If you define `+guard.server.js` (see <Link href="/file-env" />) then your `guard()` hook is:
 - Always called on the server-side.
 - Only loaded on the server-side.
