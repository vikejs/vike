...
Defining `+guard.client.js` isn't currently supported.

Generally `+guard` is used to avoid the rendering process of a page (typically when the user isn't authorized to view that page).

Usually, the motivation for defining a client-only `+guard.client.js` is to implement authorization for a pre-rendered page (e.g. an SSG app that doesn't even have a server). But that cannot work: if the page is pre-rendered then it's rendered at build-time but authorization is done at request-time. In other words: the rendering of the page (done at build-time) cannot be aborted.

So, instead consider using in another hook such as <Link href="/onHyrdationEnd">`+onHyrdationEnd`</Link> `+guard.client.js` 
The fundamental issue with `+guard.client.js`
