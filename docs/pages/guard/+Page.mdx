...
Upon the first page the user visits, `+guard.client.js` is called before rendering/<Link href="/hydration">hydrating</Link>. If that's an issue, consider:
 ...
 - Using <Link href="/render">`throw render()`</Link> / <Link href="/redirect">`throw redirect()`</Link> inside <Link href="/hooks">another hook</Link>, for example <Link href="/onHydrationEnd">`+onHyrdationEnd`</Link>.
