...
> Whenever possible, we recommend using `throw render()` / `throw redirect()` before fetching data. (Unauthorized data fetching can be problematic.)
...
If you define `+guard.server.js` then `guard()` is always executed and only loaded on the server.

> See also: see <Link href="/file-env" />

#### `+guard.client.js`

If you define `+guard.client.js` then `guard()` is executed only on the client-side and is executed on both:
- Page reload (first page visit)
- Client-side navigation

This is useful for guards that need to access browser-only APIs or should run after the page has been hydrated.

```ts
// /pages/admin/+guard.client.ts
...
async function guard(pageContext: PageContextClient) {
  // This runs on the client-side for both page reload and navigation
