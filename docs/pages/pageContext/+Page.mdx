...
It's available in all <Link href="/hooks">hooks</Link>, such as <Link href="/data">`data()`</Link>:
...
export function data(pageContext) {
  ...
  console.log('URL:'             , pageContext.urlOriginal)
  console.log('Route parameters:', pageContext.routeParams)
  console.log('HTTP headers:'    , pageContext.headers)
}
```

It's also available in any UI component with <Link href="/usePageContext">`usePageContext()`</Link>.

You can <Link href="#custom">add custom properties</Link> to `pageContext`.
