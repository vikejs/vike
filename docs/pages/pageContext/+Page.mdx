...
<Warning>
While the following is more succinct, the server code isn't eliminated when building client-side production code (see warning <Link href="#:~:text=pageContext.isClientSide,-%3A">here</Link>). Thus we recommend the approach using `import.meta.env.SSR` and `assert()` instead.
```ts
import type { PageContext } from 'vike/types'
