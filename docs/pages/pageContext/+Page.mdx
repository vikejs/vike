...
import { PropPageContext as Prop, ConfigSpec, TreeShaking } from '../../components'
...
You can access `pageContext`:
 ...
 - On the server, and on the client (using <Link href="/passToClient">`passToClient`</Link>).
...
When the user navigates to a new page, a completely new `pageContext` object is created and the previous `pageContext` becomes obsolete &mdash; that's why it's called **page**Context (and not appContext). See <Link href="#lifecycle" /> for more information.
...
declare global {
  namespace Vike {
    interface Server {
      // 👉 Pick your server (for correct pageContext.req type)
...
> When using Node.js, `pageContext.req` is [`IncomingMessage`](https://nodejs.org/api/http.html#class-httpincomingmessage).

> It's only available if you use <Link href="/server">`+server`</Link> with a server that exposes the HTTP request object (e.g. Express, Fastify, or Hono — but not Cloudflare Workers). If you don't use `+server`, then <Link href="#custom">define a custom `pageContext.req` property</Link> instead.

> Vike doesn't override `pageContext.req` if you define it as a <Link href="#custom">custom property</Link>.
...
declare global {
  namespace Vike {
    interface Server {
      // 👉 Pick your server (for correct pageContext.res type)
      server: 'hono' // | 'express' | 'fastify' | 'hattip' | 'srvx' | 'elysia' | 'h3'
    }
  }
}
...
> When using Node.js, `pageContext.res` is [`ServerResponse`](https://nodejs.org/api/http.html#class-httpserverresponse).

> It's only available if you use <Link href="/server">`+server`</Link> with a server that exposes the HTTP response object (e.g. Express, Fastify, or Hono — but not Cloudflare Workers). If you don't use `+server`, then <Link href="#custom">define a custom `pageContext.res` property</Link> instead.

> Vike doesn't override `pageContext.res` if you define it as a <Link href="#custom">custom property</Link>.

<Prop name="runtime" />

The full [`RuntimeAdapter`](https://universal-middleware.dev/reference/runtime-adapter) object set by <Link href="/server">`+server`</Link> (which uses [`universal-middleware`](https://universal-middleware.dev/) under the hood).

> Its HTTP request and response objects are also available as the aliases <Link href="#req" noWarning>`pageContext.req`</Link> and <Link href="#res" noWarning>`pageContext.res`</Link> — prefer using them.
...
See also: <Link href="/useHydrated" />.
...
> You can define `+onCreatePageContext.server.js` and/or `+onCreatePageContext.client.js` to set server- or client-side `pageContext` properties, see <Link href="/file-env" />.
...
pageContext.myCustomProp = { hello: 'world' } // Add or modify property
...
Custom properties can be defined at <Link href="/navigate#options">`navigate()`</Link> like this:
...
async function onSubmit() {
  await navigate('/form/success', { pageContext: { someExtra: 'value' } })
}

declare global {
  namespace Vike {
    // Or `interface PageContext` if pageContext.someExtra is also defined on the server
    interface PageContextClient {
      // Type of pageContext.someExtra
      someExtra?: string
    }
  }
}
...
If you don't use <Link href="/server">`+server.js`</Link>, you can define custom properties at <Link href="/renderPage">`renderPage()`</Link>.

> If you use `+server.js`, use <Link href="/onCreatePageContext">`+onCreatePageContext`</Link> instead.
...
- <Link href="/getPageContext">API > `getPageContext()`</Link>
