...
All runtime information about the <Link href="/config">page's configuration</Link> (for example the page's route and all its hooks).
...
> When building production code, [like `import.meta.env.SSR`](https://vite.dev/guide/ssr.html#conditional-logic), all code defined inside `pageContext.isClientSide` if- or else-blocks is eliminated for the sever-  or client-side bundles. (Using [Rollup's tree-shaking](https://rollupjs.org/faqs/#what-is-tree-shaking).)
>
...
Runtime information about your app, see <Link href="/globalContext" />.
...
<Warning>
While the following is more succinct, the server code isn't eliminated when building client-side production code (see warning <Link href="#isClientSide" noWarning>here</Link>). Thus we recommend the approach using `import.meta.env.SSR` and `assert()` instead.
```ts
import type { PageContext } from 'vike/types'
