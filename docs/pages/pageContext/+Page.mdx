...
The lifecycle of the `pageContext` object is fairly straightforward and predictable on the client and server, but for <Link href="/pre-rendering">pre-rendered pages</Link> it can be surprising (the `pageContext` object may seem "outdated").
...
Upon <Link href="/client-routing">client-side navigation</Link>, a new `pageContext` object is created.

> On the client-side, you can use `pageContext.previousPageContext` to access the `pageContext` of the previous page.
...
> Each pre-rendered page has a `pageContext` object saved at `dist/client/${urlOfThePage}/index.pageContext.json`.
...
>   }
>   ```
> - If the number of `?filter=` values isn't too large then, instead of `?filter=`, you can use a <Link href="/routing#parameterized-routes">paramterized route `/products/@filter`</Link> together with <Link href="/onBeforePrerenderStart">`onBeforePrerenderStart()`</Link> for pre-rendering all filter values: `/products/computer`, `/products/car`, ...
