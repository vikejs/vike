...
export const ServerOnlyNote = ({ prop, kind }) => (
  <blockquote>
    <p>
      It's only available if you use <Link href="/server"><code>+server</code></Link> with a server that exposes the HTTP {kind} object (e.g. Express, Fastify, or Hono — but not Cloudflare Workers). If you don't use <code>+server</code>, you can <Link href="#custom">define a custom <code>pageContext.{prop}</code> property</Link> yourself.
    </p>
  </blockquote>
)

export const AliasNote = ({ prop }) => (
  <blockquote>
    <p>
      <code>pageContext.{prop}</code> is an alias of <Link href="#runtime" noWarning><code>pageContext.runtime.{prop}</code></Link>.
    </p>
  </blockquote>
)
...
<ServerOnlyNote prop="req" kind="request" />
...
<ServerOnlyNote prop="res" kind="response" />
...
> It's only available if you use <Link href="/server">`+server`</Link>.

> See also:
> - <Link href="#req" noWarning>`pageContext.req`</Link>
> - <Link href="#res" noWarning>`pageContext.res`</Link>
