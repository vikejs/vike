...
Built-in properties:
 ...
 - **`pageContext.isHydration`**: Whether the page is rendered to HTML.
 ...
 - **`pageContext.isClientSide`**: Whether the page is being rendered on the client-side or server-side / <Link href="/pre-rendering">pre-rendered</Link>.
   ...
   <Warning>
     We recommend using [Vite's `import.meta.env.SSR`](https://vite.dev/guide/env-and-mode.html#built-in-constants) whenever possible instead.

     The advantage is that, when building client-side code, Vite eliminates all code defined inside `import.meta.env.SSR` if-blocks. Potentially saving a *lot* of KBs and reducing the risk of exposing sensitive server-side information. Unlike `import.meta.env.SSR` code isn't eliminated inside `!pageContext.isClientSide` if-blocks.
     ```js
 ...
 - **`pageContext.isBackwardNavigation`**: Whether the user is navigating back in history.
 ...
 - **`pageContext.isClientSideNavigation`**: Whether the page was navigated by the client-side router.
 ...
 - **`pageContext.isPageContext`**: Always `true`, useful for distinguishing `pageContext` from other objects and narrowing down TypeScript unions.
...
<Warning>
While the following is more succinct, it doesn't eliminate server code on the client-side (see warning <Link href="#:~:text=pageContext.isClientSide,-%3A">here</Link>). Thus we recommend the approach using `import.meta.env.SSR` and `assert()` instead.
```ts
import type { PageContext } from 'vike/types'

// ❌ Discouraged
function someFunction(someObject: PageContext) {
  if (someObject.isClientSide) {
    // someObject is `PageContextClient`
  } else {
    // someObject is `PageContextServer`
  }
}
```
</Warning>
