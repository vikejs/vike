...
export const ServerOnlyNote = ({ prop, kind }) => (
  <blockquote>
    <p>
      It's only available if you use <Link href="/server"><code>+server</code></Link> with a server that exposes the HTTP {kind} object (e.g. Express, Fastify, or Hono — but not Cloudflare Workers). If you don't use <code>+server</code>, then <Link href="#custom">define a custom <code>pageContext.{prop}</code> property</Link> instead.
    </p>
  </blockquote>
)
