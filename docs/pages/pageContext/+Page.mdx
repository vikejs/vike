...
The [`RuntimeAdapter`](https://universal-middleware.dev/reference/runtime-adapter) object set by <Link href="/server">`+server.js`</Link> (which uses [`universal-middleware`](https://universal-middleware.dev/) under the hood).

> It's only available if you use `+server.js`.
...
See also:
- <Link href="/server#pagecontext-runtime" doNotInferSectionTitle />
