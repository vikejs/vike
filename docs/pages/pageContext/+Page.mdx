...
<h3 id="Page"><code>pageContext.Page</code></h3>
...
The route parameters. For example, `pageContext.routeParams.movieId` for a page with a Route String `/movie/@movieId`.
...
**Server-side**:  
`pageContext.urlOriginal` is the value you passed in your server middleware:
...
- With <Link href="/client-routing">Client Routing</Link>: `window.location.href`
- With <Link href="/server-routing">Server Routing</Link>: `undefined` (unless you use <Link href="/passToClient">passToClient</Link>)
