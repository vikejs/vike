...
> This is a soft breaking change: the `0.4.22` interface is still supported, i.e. most apps should work without any change.
...
> The `onBeforeRoute()` and `onBeforePrerender()` hooks are usually only used for <Link href="/i18n" />.
