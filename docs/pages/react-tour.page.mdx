...
async function render(pageContext) {
  ...
  return escapeInjections`<!DOCTYPE html>
    <html>
      ...
      <body>
        <div id="page-view">${escapeInjections.dangerouslySkipEscape(viewHtml)}</div>
      </body>
    </html>`;
}
```
...
The `render()` hook in `_default.page.server.jsx` gives us full control over how our pages are rendered to HTML,
and `_default.page.client.jsx` gives us full control over the browser-side code.
...
even render a page with a completely different view framework, e.g. Vue or another React version (for progressive React version upgrade).
