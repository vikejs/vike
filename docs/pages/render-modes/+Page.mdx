...
<Advanced>
  This page documents how to implement render modes yourself. It's an advanced task and we generally recommend simply using the <Link href="/ssr">`ssr`</Link> and <Link href="/pre-rendering#get-started">`prerender`</Link> settings instead.
</Advanced>
...
> What "SPA", "SSR", "HTML-only" and "SSG" mean, and which one should be used, is explained at <Link href="/SSR-vs-SPA" />.
...
If we want to have both SSR and SPA pages, then see <Link href="#spa">the SPA section</Link>.
...
<Warning>Using modern UI frameworks (React/Vue/...) to render pages only to HTML is an uncommon technique.</Warning>
...
function Page() {
  return (
    <>
      ...
      <p>This page is rendered only to HTML. (It's not loaded/rendered in the browser-side.)</p>
    </>
  )
}
```
