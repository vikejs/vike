...
For files that are not processed by Vite (e.g. <RepoLink path="/boilerplates/boilerplate-react/server/index.js" text="Express.js server code"/>) we can use a Node.js aliasing tools such as [`module-alias`](https://github.com/ilearnio/module-alias) or write it in `"imports"` section of `package.json` (see docs [here](https://nodejs.org/api/packages.html#packages_subpath_patterns)). All browser files are always processed by Vite.
