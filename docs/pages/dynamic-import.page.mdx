...
UI frameworks usually have built-in support for dynamically loading components:
 - [React Docs > Code-Splitting > `React.lazy`](https://reactjs.org/docs/code-splitting.html#reactlazy) (despite the warning, React's SSR support for `<React.Suspense>` is already stable and used in production by many companies)
