...
async function startServer() {
  ...
  app.get('*', async (req, res, next) => {
    ...
    // Optionally, we can provide additional information about the request.
    // Such as information about the logged-in user (e.g. when using an
    // Express.js authentication middleware), or the User Agent header.
    pageContextInit.user = req.user
    pageContextInit.userAgent = req.headers['user-agent']
