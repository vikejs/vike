...
The `renderPage()` function enables you to embed Vike into any server, see <Link href="/integration#server-manual-integration">Server (manual integration)</Link>.

> We recommend using <Link href="/server">`+server.js`</Link> instead of directly using `renderPage()` yourself.
