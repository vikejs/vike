...
The `renderPage()` function enables you to embed Vike into any server, see <Link href="/server-integration#manual-integration" />.

> We recommend using <Link href="/vike-server">`vike-server`</Link> instead of directly using `renderPage()` yourself.
