...
If you <Link text="pre-render" href="/pre-rendering" /> all your pages then you don't need to use `renderPage()`, because:
 - Upon development, Vike automatically embeds itself into Vite's development server (`$ vike`/`$ vike dev`).
 - Upon pre-rendering (`$ vike build`/`$ vike prerender`) Vike renders all your pages.
