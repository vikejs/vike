...
> For example, with Vike, you have <Link href="/data#client-side">fine-grained control over where data is fetched</Link>: on the server-side, client-side, or both. It's a small but essential feature, as it's one aspect of Vike's first-class support of both:
