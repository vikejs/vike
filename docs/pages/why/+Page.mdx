...
> If you are curious, see <Link href="/architecture" /> for an explanation of why Vike's design is flexible.
...
> - Start with <Link href="/RPC">RPC</Link>, then progressivley swtich to GraphQL if/when the need arises.
> - Start a JavaScript backend then, if the need arises, progressivley switch to a highly-scalable backend such as Elixir.
