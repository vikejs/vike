...
[Auth.js](https://next-auth.js.org/) (previously known as NextAuth.js) can be used with `vite-plugin-ssr`.
...
<Note type='warning'>Don't add `NEXTAUTH_` to `envPrefix` as it would leak `NEXTAUTH_SECRET`.</Note>

See also:
- [Vite > Config > Environment Variables](https://vitejs.dev/config/#environment-variables)

> We can use `process.env` as usual for server (e.g. Express.js) code. (Since [server code isn't processed by Vite](https://github.com/brillout/vite-plugin-ssr/issues/562).)
...
> A polyfill is not needed if we use `getSession()` only on the client-side, or if we use Node.js 18 which [natively supports `fetch()`](https://nodejs.org/en/blog/announcements/v18-release-announce/#fetch-experimental).
