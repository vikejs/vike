...
We (<Link href="/team">Joël, Dani, and Rom</Link>) have been working on a <Link href="/server">new Vike extension `vike-server`</Link> which integrates Vite with any server (Express.js, Hono, Fastify, Elysia, H3, ...) and any deployment (VPS, Netlify, Cloudflare, Vercel, ...).
