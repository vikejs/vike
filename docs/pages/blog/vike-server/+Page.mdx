...
> Using Vite to transpile server code was one of the most requested features (unsurprisingly so). In fact, we considered this Vike's last significant shortcoming: Vike can now be considered a state-of-the-art and full-fledged framework and we're now a lot <Link href="#releasing-1-0">closer to releasing `1.0`</Link>!
