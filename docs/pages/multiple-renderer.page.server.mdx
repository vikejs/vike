...
marketing/+config.h.js # (Route to `/*` instead of `/marketing/*`.)
...
(We use `+config.h.js` to change the Filesystem Routing Root, see <Link href="/filesystemRoutingRoot" />.)
...
# We only override `+onRenderClient.js` while `+onRenderHtml.js` stays the same.
