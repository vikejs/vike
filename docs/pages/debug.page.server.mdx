...
If you have a problem with Vite, we recommend to consider injecting a bunch of `console.log()` inside `node_modules/vite/dist/**/*.js` to inspect what's going on.

> Instead of `$ git clone git@github.com:vitejs/vite && cd vite/ && npm install && npm run buid && cd ../path/to/my-app/ && npm link ../vite/packages/vite/`, we recommend to:
...
> - You may end up being able to do a Vite PR solving your problem. (If you open a Vite PR, feel free to reach out to the `vite-plugin-ssr` maintainers to get your PR merged quicker.)
