...
> These debug logs allow you to understand how `vike` thinks of your app. For example, to figure out why your app returns a `404`, use `vps:routing` and `vps:pageFiles`.

There are more debug logs (`vps:glob`, `vps:extractAssets`, ...) but these aren't user friendly as they are meant for vike maintainers.

> If you want more debug logs, add a comment at [#249](https://github.com/brillout/vike/issues/249).
