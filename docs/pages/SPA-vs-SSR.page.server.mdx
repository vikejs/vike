...
Technically and precisely speaking, the difference between each render mode is the following.
 ...
 - **SPA** (aka MPA): the page is only rendered in the browser. The page's content is not rendered to HTML. (The page is loaded only in the browser.)
...
MPA is more or less the same than SPA. In the context of `vite-plugin-ssr`, we use both terms interchangeably.

> MPA denotes "Multiple Page App" while SPA denotes "Single Page App". So the only difference is that SPA apps only have one page, which is a distinction that doesn't make much sense (especially in the context of `vite-plugin-ssr`).
