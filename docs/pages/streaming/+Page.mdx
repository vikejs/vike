...
async function handleFetchEvent(event: Event) {
...
> Instead of using HTML streaming, an easy alternative is to use a stateful component. But this approach has a major issue: content isn't rendered to HTML.
>
> For example, if a product page shows content from a database, then the content of that product page won't show in the HTML.
>
> This means you miss <Link href="/SSR-vs-SPA">all the SEO and performance benefits of SSR</Link>.
>
> With HTML streaming, all content — even content loaded later — is rendered to HTML.
