...
Some UI frameworks, such as React, support *progressive rendering*: while some parts of the UI are being loaded, other parts are already rendered.

(as well as <Link href="/hydration">hydrated</Link>!).

<video src={progressiveRenderingDemo} width="500" controls muted autoPlay loop></video>

> Instead of using HTML streaming, an easy alternative is to use a stateful component. But the issue is that the content that is rendered later isn't rendered to HTML. For example, a product page with content fetched from a database won't get <Link href="/SPA-vs-SSR#ssr">the SEO nor performance advantages of SSR</Link>.
>
> With streaming all content, including the content that is rendered later, is rendered to HTML.
