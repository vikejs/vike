...
> Instead of using HTML streaming, an alternative and easy way to achieve this is to use a stateful component. But the issue with that approach is that the content that is rendered later isn't rendered to HTML. For example, a product page with content fetched from a database will be missing out on the SEO and performance perks of SSR.
>
> The benefit of using HTML streaming to implement progressive rendering is that all content, even the content that is rendered later, is rendered to HTML.
