...
<Danger>Support for executing more than one `onBeforeRender()` hook is deprecated. Use <Link href="/exports" text="Custom Exports/Hooks"/> instead. (You can still define multiple `onBeforeRender()` hooks but only one will be executed.)</Danger>
