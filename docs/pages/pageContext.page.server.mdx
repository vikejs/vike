...
## Built-in
 ...
 - **`pageContext.isHydration`**: Whether the current page is already rendered to HTML. In other words, when using <Link href="/client-routing" noBreadcrumb={true} />, the value is usually `true` for the first page the user navigates to, and `false` for any subsequent navigation. (When using <Link href="/server-routing" noBreadcrumb={true} />, the value is always `true`.)
