...
Upon server-side rendering, the `pageContext` object lives only during the creation of the HTTP response. (You shouldn't retain the `pageContext` object.)

Upon client-side page navigation, the `pageContext` object of the previous page is discarded and a new `pageContext` object is created.
