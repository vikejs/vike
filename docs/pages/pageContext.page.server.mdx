...
On the server-side, upon server-side rendering, a new `pageContext` object is created and used only for/during the creation of the HTTP response. (You shouldn't use the `pageContext` object once the HTTP response is sent.)
