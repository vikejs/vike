...
What happens with the `Page` value is determined by the <VikeExtensionUiFramework />. Alternatively, for complete control over the UI framework integration, you can <Link href="#manual-integration">manually integrate</Link> the UI framework yourself.
...
The <VikeExtensionUiFramework /> implements the `onRenderHtml()` and `onRenderClient()` hooks, so that you don't have to implement them: everything just works like a regular framework such as Next.js or Nuxt.

Instead of using <VikeExtensionUiFrameworkNames />, you can integrate your favorite UI framework yourself: you then have complete control over the UI framework integration.
