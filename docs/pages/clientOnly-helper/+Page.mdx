import { Link, Warning, Danger, Tabs } from '@brillout/docpress'
import { UiFrameworkExtension, ConfigSpec } from '../../components'

<ConfigSpec
  env="client, server"
  providedByExtension={{ kind: 'helper', name: 'clientOnly()' }}
  global={null}
/>
...
<Danger>
  The `clientOnly()` will soon be deprecated, use <Link href="/ClientOnly">`<ClientOnly>`</Link> instead.
</Danger>
