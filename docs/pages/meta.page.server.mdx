...
You can use `meta` to change the environment in which built-in configs are loaded. For example you can change `onBeforeRender()` from `{ server: true, client: false }` to `{ server: true, client: true }`.
