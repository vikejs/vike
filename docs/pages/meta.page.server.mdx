...
export default {
  meta: {
    someSettingOrHook: {
      ...
      env: {
        //  Load the config value on the server.
        server: true,
        //  Load the config value on the client.
        client: true,
        // Load the config value at config time. Required for `effect`. Optional.
        config: false // default value
      },

      // Optional. Whether config values should be merged instead of overwritten.
      cumulative: false, // default value

      // Optional. Function called when the config value is defined.
...
// in a .d.ts file then make sure there is at least one export/import statment.
// Tell TypeScript this file isn't an ambient module:
export {}
```

```ts
// /pages/product/@id/+config.h.ts
