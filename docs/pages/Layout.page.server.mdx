...
export const '/product/@id/@view'
```

> Instead of a <Link href="/route-string">Route String</Link>, you can use a <Link href="/route-function">Route Function</Link> for full programmatic flexibility, see [this example](https://gist.github.com/brillout/a75fd8430dbc562dd10e69007121fea0).


To ensure `@view` matches a list pre-defined values, use a <Link href="/guard">`guard()`</Link> hook with <Link href="/render">`throw render(404)`</Link>:
...
You then use `pageContext.routeParams.view` to route the nested layout:
...
Finally, make sure to use <Link href="/clientRouting#settings">`<a href="/product/42/reviews" keep-scroll-position />`</Link> (or <Link href="/navigate#options">`navigate('/product/42/reviews', { keepScrollPosition: true })`</Link>) to avoid the browser to scroll to the top.
...
For smooth nested layout navigation, we recommend using <Link href="/clientRouting">Client Routing</Link>. (Using <Link href="/server-routing">Server Routing</Link> would lead to a full page reload upon nested layout navigation which usually isn't an acceptable UX.)
