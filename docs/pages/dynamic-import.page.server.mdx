...
The most common use cases are:
 - **Performance**: we can use a dynamic `import()` to defer loading heavy components (e.g. a heavy interactive map), so that the user can already interact with our page before even the browser has started loading that heavy component.
 - **Client-side only components**: we can use a dynamic `import()` to load & render a component only on the client-side. (Some components don't work in Node.js, see <Link href="/client-only-components" />.)
   > Most UI Frameworks's (e.g. React and Vue) built-in support for dynamically loading a component execute the dynamic `import()` only on the browser-side. In other words, compoments loaded that way are never loaded on the server-side.

UI frameworks usually have built-in support for dynamically loading a component with `import()`:
