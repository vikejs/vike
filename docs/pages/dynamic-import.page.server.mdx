...
Common use cases:
 - **Performance**: we can use a `import()` to defer loading a heavy component (e.g. a heavy interactive map), so that the user can already interact with our page before even the browser has started loading that heavy component.
 - **Client-side only components**: we can use a `import()` to load & render a component only on the client-side. (Some components don't work in Node.js, see <Link href="/client-only-components" />.)
