...
Common use cases:
 ...
 - **Client-side only components**: we can use `import()` to load & render a component only on the client-side. (Some omponent libraries cannot be server-side rendered, see <Link href="/client-only-components" />.)
   > The UI frameworks's built-in support for such lazy-loaded component usually execute the dynamic `import()` only on the browser-side (e.g. React and Vue), i.e. the lazy compoment is never loaded on the server-side.
