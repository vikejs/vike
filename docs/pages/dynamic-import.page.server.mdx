...
function Map() {
  ...
  React.useEffect(() => {
  // Typescript Implementations: Error can be ignored as lazy() return type should be a valid functional component
  //@ts-ignore: Type 'LazyExoticComponent<ComponentType<any>>' is not assignable to type '(() => Element) | Element'.
