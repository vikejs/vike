...
The `vike-react` extension doesn't implement the `clientOnly()` helper (yet). Use the <Link href="/ClientOnly#react">`<ClientOnly>`</Link> wrapper instead.

> Contribution welcome, see [#102 - `clientOnly()` instead of `<ClientOnly>`](https://github.com/vikejs/vike-react/issues/102)
