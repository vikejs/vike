...
You can use `+Head` for settting `<head>` tags are read by HTML crawlers:
...
## Injecting arbitrary HTML strings

You can inject any arbitrary HTML string to the page's `<head>`. For example using:
...
You can use React's `dangerouslySetInnerHTML` to add any arbitrary HTML string, for example:
...
You can use `innerHTML` to add any arbitrary HTML string, for example:
