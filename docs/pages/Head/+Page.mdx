...
Environment: server.
<ImplementedBy noCustomGuide>it</ImplementedBy>

The `Head` setting allows you to add `<head>` tags to your pages.

> See <Link href="/head" /> for a general introduction and other ways to add `<head>` tags.
...
## Access data / `pageContext`

You can set `<head>` tags based on fetched data (or <Link href="/pageContext">`pageContext`</Link>) by:

- Using the component hooks `getPageContext()` and `getData()` inside `+Head`
- Using <Link href="/useConfig">`useConfig()`</Link> inside <Link href="/data">`+data`</Link>.
- Using `useConfig()` inside any component.

**`getPageContext()` and `getData()`**

You can use <Link href="/usePageContext">`usePageContext()`</Link> and <Link href="/useData">`useData()`</Link> as usual:
...
Because `+Head` The `<Head>` 


- Using the `<Head>` component (not `+Head`) inside any component.
  > There are two different `<Head>` components:
    - One defined by `+Head` (as documented throughout this page).
    - Another one exported 
...
You can use `useData()` as well `usePageContext()` inside the `<Head>` component:
...
The most notable limitation is that the `Head` setting cannot be used to set the `<title>` tag, because the title isn't updated upon client-side navigating between two pages that have a different title.
...
For example, in the following, setting `<title>` won't work while setting.
...
Note that this isn't an issue for `<meta name="description">` tag because it's meant for search engines bots which
