...
> The `+Head` setting **only applies to the HTML** of the first page the user visits.
>
> As explained at <Link href="#only-html" />, you should use `+Head` only for `<head>` tags that are either:
> - intended for **crawlers** (e.g. SEO or social sharing tags), or
> - **static** (e.g. favicon or PWA settings that don't change between pages).
>
> You cannot use it for dynamic tags intended for the user's browser such as `<title>` (use <Link href="/title">`+title`</Link> instead).
...
**Only rendered for the first page's HTML**
