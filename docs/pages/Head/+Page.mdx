...
The most notable limitation is that the `Head` setting cannot be used to set the `<title>` value, because the title isn't updated upon client-side navigating to a page that has a different title.

> See <Link href="#example">example below</Link> for a more detailed explanation.

For use cases where the `Head` setting cannot be used, Vike offers settings that update upon client-side navigation such as <Link href="/title">`title`</Link>.
...
You can use the `Head` setting for `<head>` tags are read by HTML crawlers:
...
You can use the `Head` setting for `<head>` tags are global (they have the same value for you all your pages):
...
The following example showcases that using the `Head` setting for `<title>` doesn't work while it does work for `<meta name="description">`.
