...
The HTML sent to the browser is just an empty shell that delivers the page's client-side JavaScript:
...
> In this example, hydration is unnecessary since `<Welcome>` isn't interactive. However, interactive components (e.g. `<Counter>`) require hydration to function properly.
...
- **Crawlers** (search engines, social media, AI)
- **Performance** (particularly on mobile devices and slow connections)


## Crawlers

The [crawlers](https://en.wikipedia.org/wiki/Web_crawler) of **search engines** (e.g. Google, Bing), **social media platforms** (e.g. Instagram, Twitter), and **AI** (e.g. ChatGPT) navigate your website by reading the HTML of your pages and following `<a href="/some-other-page">` links.
