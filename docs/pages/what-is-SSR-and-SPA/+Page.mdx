...
This JSX code is compiled (by [Vite](https://vite.dev)) to JavaScript and can run on both the client and the server.
...
> **Note:** Google's crawler can execute client-side JavaScript, but we still recommend using SSR even if you only target Google.
>
> <details>
...
> **Note:** A more accurate term would be CSR (Client-Side Rendering), but "CSR" is rarely used while "SPA" is widely used in the web development community. That's why we use the term SPA throughout Vike's documentation.
