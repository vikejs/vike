...
A unique aspect about JavaScript is that it can run in both the browser (browsers support only two languages: [JavaScript](https://en.wikipedia.org/wiki/JavaScript) and [WebAssembly](https://webassembly.org)) and on the server (by using a JavaScript runtime such as [Node.js](https://nodejs.org)).
...
>
> Google's crawler is unique because it can execute client-side JavaScript, unlike most other search engines and social media crawlers. This means Google can potentially index SPA content, but there are several limitations:
>
> - **Unreliable execution**: JavaScript execution may fail or timeout
...
> - **Future uncertainty**: Google may discontinue JavaScript execution as SSR becomes standard
>
> For these reasons, SSR remains the recommended approach even for Google-only SEO.
> </details>
...
> **Note:** A more accurate term would be CSR (Client-Side Rendering), but "SPA" is widely used in the web development community. That's why we use the term SPA throughout Vike's documentation.
