...
async function data(pageContext) {
  ...
  // We make `movies` available as pageContext.data
  return {
    movies
  }
}
...
async function onRenderClient(pageContext) {
  // Because pageContext.data is part of `passToClient` by default, it is
...
Fetching data can also be achieved with the more advanced, lower-level <Link href="/onBeforeRender">`onBeforeRender()`</Link>
hook.
...
For more convenience, instead of creating a new `data()` hook for each page, we can define `data()` only once in `/renderer/+data.js` while <Link text="creating new settings" href="/meta" />.
