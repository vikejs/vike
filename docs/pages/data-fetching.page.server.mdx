...
For more convenience, instead of creating a new `onBeforeRender()` hook for each page, we can define `onBeforeRender()` once in `/renderer/_default.page.server.js` while using a <Link text="Custom Export/Hook" href="/exports" />.
