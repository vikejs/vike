...
For more convenience we can define `onBeforeRender()` in `_default.page.server.js` and use a <Link text="Custom Export" href="/exports" />.
...
async function onBeforeRender(pageContext) {
  // The export values above are available at `pageContext.exports.query`
...
> If the logic differs substantially between two parts of our app, we can also create <Link text={<>mutliple <code>renderer/</code> directories</>} href="/multiple-renderer" />.
