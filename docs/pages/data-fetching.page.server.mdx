...
async function onBeforeRender(pageContext) {
  // `+onBeforeRender.js` runs only in Node.js by default; we could use SQL/ORM queries here.
...
// In the `onRenderHtml()` and `onRenderClient()` functions above, we pass
// `pageContext.pageProps` to `Page`
...
async function onBeforeRender(pageContext) {
  // The @id in /pages/movie/@id/+onBeforeRender.js denotes a route parameter
...
For more convenience, instead of creating a new `onBeforeRender()` hook for each page, we can define `onBeforeRender()` only once in `/renderer/+onBeforeRender.js` while using a <Link text="Custom Export/Hook" href="/exports" />.
...
async function onBeforeRender(pageContext) {
  // Our `query` export values are available at `pageContext.query`
...
The `+onBeforeRender.js` files are loaded only in Node.js;
...
then we have the option to [also run `onBeforeRender()` in the client](/onBeforeRender-isomorphic), i.e it is not only
called in Node.js but also in the browser (upon page navigation). Although
[not recommended](/onBeforeRender-isomorphic#recommendation), this can be achieved by
[modifying the `meta` config](/meta#modify-existing-configurations).
