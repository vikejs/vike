...
We should keep in mind that simpler app architecture => higher development speed => more features for our users. Do our users care more about features or about faster page transitions?

For example, as a startup that wants to deliver an MVP as quickly as possible, Server Routing should be the default choice.
...
We recommend using Client Routing only with a rationale, for example:
 ...
 - Web apps that need to **maintain state across page changes**. For example Spotify: the currently listened song should not be interrupted on page change. Server Routing does not work for these kind of apps and we need to use Client Routing.
