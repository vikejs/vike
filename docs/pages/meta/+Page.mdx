...
export default (pageContext: PageContextServer) => pageContext.data.product.name
```
```ts
...
function getTitle(pageContext: PageContextServer | PageContextClient) {
...
function getDescription(pageContext: PageContextServer | PageContextClient) {
  // Same as getTitle()
  // ...
}
```
