...
> If you use a <UiFrameworkVikeExtension/>, then you don't need to create the `title` and `description` settings as they are already created by <UiFrameworkVikeExtension name />. You can still read this section as it's a good example for showcasing `meta`.
...
> If you use a <UiFrameworkVikeExtension/>, then you don't need to create the `Layout` setting as it's already created by <UiFrameworkVikeExtension name />.
