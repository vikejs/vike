...
<Advanced>
  Using `meta` is an advanced feature. Consider using the following instead:
  - <Link href="/settings">Settings</Link>. (Some of them, such as <Link href="/ssr">`ssr`</Link>, will set `meta` on your behalf.)
  - <Link href="/extensions" noBreadcrumb={true} />. (They use `meta` under the hood.)
