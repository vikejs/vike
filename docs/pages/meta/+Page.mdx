...
<Advanced>
  The `meta` setting is an advanced feature. Instead of directly using `meta` yourself, consider using <Link href="/file-env" noBreadcrumb={true} /> or <Link href="/extensions">Vike Extensions</Link> (which use `meta` under the hood).
</Advanced>
