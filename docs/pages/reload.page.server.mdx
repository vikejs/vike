...
The `reload()` function enables you to reload the page. The difference with [`window.location.reload`](https://developer.mozilla.org/en-US/docs/Web/API/Location/reload) is that `reload()` is much faster as it uses vite-plugin-ssr's <Link text="Client Router" href="/client-routing" />.
