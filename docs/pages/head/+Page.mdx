...
> You can dynamically set these settings:
> - Inside your `+data` hook, by using the <Link href="/useConfig">`useConfig()` hook</Link>.
> - Inside your components, by using the <Link href="/useConfig#config-head">`<Head>` component</Link>.
...
> As explained at <Link href="/config#files"/> defining the `title` setting with a `+title.js` file has the same effect as defining it inside `+config.js`.
