...
> Unlike the other settings, the `Head` setting is cumulative which means that `pages/+Head.js` applies to all your pages and you cannot override it, see <Link href="/Head-setting#cumulative"></Link>.


### Override defaults

You can set different `<head>` tags for specific pages, overriding the default.
