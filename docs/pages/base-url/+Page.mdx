...
> For example, instead of serving your app at `https://example.org/*` (the Base URL is `/`), you can serve your website at `https://example.org/some-base/*` by setting the Base URL to `/some-base/`.
...
Note how the Base URL of the URL of your pages isn't changed: your app is still served at `https://my-website.com/*` (the Base URL is still `/`).
