import { Link } from '@brillout/docpress'
import { CommunityNote, Example } from '../../components'

<CommunityNote url="https://storybook.js.org" />

[Storybook](https://storybook.js.org) is a frontend workshop for building UI components and pages in isolation.

## Installation

Run the following command in your project's root directory:

```sh
npx storybook@latest init
```

Storybook auto-detects your setup. When prompted for a framework, pick the one matching your UI framework:

| UI Framework | Storybook Framework |
|---|---|
| React | `@storybook/react-vite` |
| Vue | `@storybook/vue3-vite` |
| Svelte | `@storybook/svelte-vite` |
| Solid | `@storybook/solid-vite` |
| Preact | `@storybook/preact-vite` |

After installation, run Storybook:

```sh
# Development server (http://localhost:6006)
storybook dev -p 6006

# Production build
storybook build
```

We designed Vike so that using Vike with Storybook works out of the box.

> By default, <Link href="/vite-plugin">Vike's Vite plugin</Link> isn't loaded when running Storybook.
>
> This is because Storybook is typically used for developing/testing components in isolation — Vike's Vite plugin isn't needed. For that reason, Vike's Vite plugin removes itself when it detects Storybook.
>
> If for some reason you do need Vike's Vite plugin to be loaded, reach out to Vike maintainers.


## Example

 - <Example timestamp="2026.02" repo="brillout/vike-storybook-example" />


## See also

- [Storybook > Vite](https://storybook.js.org/docs/builders/vite)
