...
but we should do so only if we have a strong rationale:
the solution using `pageContext.documentProps` (see [View Component](#view-component)) is dramatically simpler and works for *all* use cases we are aware of.
