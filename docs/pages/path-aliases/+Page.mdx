...
If you don't use <Link href="/vike-server">`vike-server`</Link> then your server files aren't processed by Vite — they are directly executed by Node.js. Therefore, you must configure Node.js to resolve your path aliases:
...
> - Server files, if you use <Link href="/vike-server">`vike-server`</Link>.
