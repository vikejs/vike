...
await dev({
  viteConfig // Vite settings (optional)
})
console.log('Development server is ready')
```

See <Link href='#viteconfig' />.
...
<Link href="/pre-rendering">Pre-render</Link> pages (only needed when <Link href="/prerender#disableautorun">`partial.disableAutoRun`</Link> is `true`).
...
Initial <Link href="/pageContext">`pageContext`</Link> values.
