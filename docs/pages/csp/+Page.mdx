...
If `+csp.nonce` is `true`, Vike generates a random string for each HTTP request, stores it in `pageContext.cspNonce`, and appends it to `<script>` tags.
...
Instead of using the `+csp` setting, you can directly set `pageContext.cspNonce`:
...
You can also use the `+csp` setting while modifying `pageContext.cspNonce` — Vike uses the value set at `pageContext.cspNonce` as the nonce value.
