...
Hook called for initializing <Link href="/pre-rendering">pre-rendering</Link> (before <Link href="/onPrerenderStart" />).

> It's executed only for pre-rendering and, consequently, never executes during development.
