...
When a page `some.page.js` is rendered, up to four different `onBeforeRender()` hooks can be called:
 - `export { onBeforeRender }` of `_default.page.js`
 - `export { onBeforeRender }` of `_default.page.server.js`
 - `export { onBeforeRender }` of `some.page.js`
 - `export { onBeforeRender }` of `some.page.server.js`

We have full control over the execution flow.

## Server Hooks

[`onBeforeRender()` hooks defined in `.page.server.js`](/onBeforeRender).
...
[`onBeforeRender()` hooks defined in `.page.js`](/onBeforeRender-isomorphic).
