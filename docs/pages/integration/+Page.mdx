...
[Google Analytics](https://developers.google.com/analytics), [Plausible](https://plausible.io), [Segment](https://segment.com)

If you want to load analytics code, consider using:
...
Some compnent libraries need initilization, most notably vanilla component libraries such as Bootstrap.

To initilizae code, you can use
 - <Link href="/onHydrationEnd" />
 - <Link href="/client" />
...
In general we recommend using data-fetching tools only for non-initial data, see <Link href="/data-fetching#initial-data" />.

Some data-fetching tools, such as <Link href="/tanstack-query">TanStack Query</Link>, enable you to <Link href="/data-fetching#tools">use tools for fetching initial data</Link>.

<Advanced>
The following documents how to manually integrate tools for fetching initial data. We recommend using a Vike extension that already does this for you.
</Advanced>
...
You can use the <Link href="/client">`+client.js`</Link> file to initialize your [service worker](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API).
