...
In general, <Link href="/server">we recommend using `vike-server`</Link> for automatically integrating Vike with your JavaScript server (Express.js, Hono, Fastify, ...). But you can manually integrate Vike into your server instead if you prefer.
