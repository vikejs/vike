...
import { IntegrationHelp, JustAMiddleware, Advanced } from '../../components'
...
2. On the client-side, initialize with `pageContext.dataInitial`.
...
async function onRenderHtml(pageContext: PageContextServer) {
  // `Page` is the `export { Page }` of our +Page.ts files;
...
To integrate Vike with a JavaScript server, we generally recommend <Link href="/server">`+server`</Link> which automatically integrates Vike with any JavaScript server (Express.js, Hono, Fastify, ...) and deployment provider (Cloudflare, Vercel, Netlify, self-hosting, ...).

But you can also manually integrate Vike into your server for full control over server and deployment integration.
