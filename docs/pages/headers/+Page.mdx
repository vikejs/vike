...
All values are merged; you can get the list of values as an array (`string[]`) by doing:
```js
...
> You can access `pageContext` in any Vike hook and any UI component, see: <Link href="/pageContext" />.

<Warning>Both `+headersResponse` and/or `pageContext.headersResponse` are beta features: expect breaking changes in any minor version update.</Warning>

The final HTTP response headers that will be sent to the client are available at `pageContext.httpResponse.headers`.
