...
To achieve that, you can create the store at <Link href="/onCreatePageContext">`+onCreatePageContext.ssr.js`</Link>  and then retrieve its initial sate at <Link href="/onAfterRenderHtml">`+onAfterRenderHtml.js`</Link>:
```ts
