...
> It's somewhat experimental so expect issues here and there. We may built it directly into Vike (instead of being a seperate package).
...
> It was already possible with <Link href="/injectFilter">`injectFilter()`</Link> but only partially: `injectScriptsAt` allows you to inject scripts at the begining of the <Link href="/streaming">HTML stream</Link>. It's also simpler to use than `injectFilter()` and it can be used by Vike extensions.
