...
<BlogHeader
  ...
  social={{
    twitter: 'https://x.com/vike_js/status/1818992472917180427',
  }}
/>
...
Not only Vue and Solid users, but now also React users can use the <Link href="/clientOnly-helper">`clientOnly()` helper</Link> which has the same functionality as the `<ClientOnly>` wrapper (now deprecated) but with a much improved DX.
