...
<BlogHeader
  ...
  social={{
    twitter: 'https://x.com/brillout/status/1807756549730783391',
  }}
/>
...
Vue users can now use the <Link href="/clientOnly-helper">`clientOnly()`</Link> helper which has the same functionality as the `<ClientOnly>` wrapper (now deprecated) but with a much improved DX.
