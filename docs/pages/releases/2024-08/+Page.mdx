...
Last month, we worked on <Link href="/head-tags">improving `<head>` management</Link> for `vike-react`.

We have done a lot of fixing & polishing and ported all improvements across `vike-{react,vue,solid}`.
...
New [Bati](https://batijs.dev) integrations:
 - [Lucia (auth library)](https://lucia-auth.com)
