...
Some data fetching tools, such as <Link href="/relay" text="Relay">Relay</Link> or [Vue's `onServerPrefetch()`](https://vuejs.org/api/composition-api-lifecycle.html#onserverprefetch), provide initial data only after the stream has ended.

In such situations, we can return a `pageContextPromise` in our `render()` hook:
...
async function render(pageContext) {
  ...
  // pageContextPromise can be a promise, or a function that
  // will be called after the stream has ended.
