...
By default, the HTML template (we provide in our `render()` hook) isn't immediately written to the stream: instead, Vike awaits for our UI framework to start writing to the stream.
