...
Examples:
 ...
 - With React 18 + [`react-streaming`](https://github.com/brillout/react-streaming) + Cloudflare Workers: <RepoLink path='/examples/cloudflare-workers-react-full/' />
...
By default, the HTML template we provided in the `render()` hook isn't immediately written to the stream: instead, `vite-plugin-ssr` awaits for your UI framework to start writing to the stream.
