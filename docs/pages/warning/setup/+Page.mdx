...
In development, if you use a server without <Link href="/server">`+server.js`</Link> (i.e. you <Link href="/integration#server-manual-integration" doNotInferSectionTitle>manually integrate your server</Link>), you must use <Link href="/createDevMiddleware">`createDevMiddleware()`</Link> in order to add the development middleware to your server. (If you use `+server.js`, the development middleware is added automatically — you don't need to use `createDevMiddleware()`.)
