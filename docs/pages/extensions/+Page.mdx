import { Link, Contribution } from '@brillout/docpress'

Vike extensions integrate tools on your behalf.

> Extensions are optional — you can integrate tools manually instead (extensions use the same API you use).
> That said, we recommend using extensions unless you have a clear reason not to — see <Link href="/extension-vs-custom" />.


## React

**Core**

- <Link href="/vike-react">`vike-react`</Link>
- [`vike-react-rsc`](https://github.com/nitedani/vike-react-rsc#readme) - 🚧 Same as `vike-react` but with <Link href="/react#react-server-components">Server Components</Link>

**Data fetching**

- [`vike-react-query`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-query#readme)
- [`vike-react-apollo`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-apollo#readme)

**State management**

- [`vike-react-redux`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-redux#readme)
- [`vike-react-zustand`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-zustand#readme)

**Error handling**

- [`vike-react-sentry`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-sentry#readme)

**UI Component Libraries**

- [`vike-react-chakra`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-chakra#readme)
- [`vike-react-antd`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-antd#readme)

**CSS-in-JS**

- [`vike-react-styled-components`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-styled-components#readme)
- [`vike-react-styled-jsx`](https://github.com/vikejs/vike-react/tree/main/packages/vike-react-styled-jsx#readme)

<Contribution>[Contributions welcome](https://github.com/vikejs/vike/issues/1715) to create extensions.</Contribution>


## Vue

- <Link href="/vike-vue">`vike-vue`</Link>
- [`vike-vue-query`](https://github.com/vikejs/vike-vue/tree/main/packages/vike-vue-query#readme)
- [`vike-vue-pinia`](https://github.com/vikejs/vike-vue/tree/main/packages/vike-vue-pinia#readme)
- [`vike-vue-pinia-colada`](https://github.com/kevinmarrec/vike-vue-pinia-colada#readme)
- [`vike-vue-vuetify`](https://github.com/Mahdi-heydari/vike-vue-vuetify#readme)
- [`vike-vue-naive-ui`](https://github.com/Jearce/vike-vue-naive-ui#readme)

<Contribution>[Contributions welcome](https://github.com/vikejs/vike/issues/1715) to create extensions.</Contribution>


## Solid

- <Link href="/vike-solid">`vike-solid`</Link>
- [`vike-solid-query`](https://github.com/vikejs/vike-solid/tree/main/packages/vike-solid-query#readme)

<Contribution>[Contributions welcome](https://github.com/vikejs/vike/issues/1715) to create extensions.</Contribution>


## Svelte

- 🚧 [`vike-svelte`](https://github.com/brandonxiang/vike-svelte#readme)

<Contribution>Rough proof-of-concept: contributions welcome to take over and finish it.</Contribution>


## Angular

- 🚧 [`vike-angular`](https://github.com/vikejs/vike-angular#readme)

<Contribution>Rough proof-of-concept: contributions welcome to take over and finish it.</Contribution>


## Server

Adapters for integrating a <Link href="/server">server framework</Link>:

- [`@vikejs/hono`](https://github.com/vikejs/servers/tree/main/packages/hono#readme)
- [`@vikejs/h3`](https://github.com/vikejs/servers/tree/main/packages/h3#readme)
- [`@vikejs/elysia`](https://github.com/vikejs/servers/tree/main/packages/elysia#readme)
- [`@vikejs/express`](https://github.com/vikejs/servers/tree/main/packages/express#readme)
- [`@vikejs/fastify`](https://github.com/vikejs/servers/tree/main/packages/fastify#readme)

<Contribution>[Contributions welcome](https://github.com/vikejs/vike/issues/1715) to create extensions.</Contribution>


## i18n

- [`vike-i18n-routing`](https://github.com/vad1ym/vike-i18n-routing) - Fully localized routing for Vike. Translated URLs, locale-aware links, redirects, and SSR-first i18n routing.


## Frameworks

- [`vike-vue-content`](https://github.com/EralChen/vike-vue-content#readme) - Content rendering framework built on Vike + Vue for docs sites with content-driven routing and zero-boilerplate setup.
- [Rudder](https://github.com/rudderjs/rudder#readme) - Laravel-inspired, framework-agnostic Node.js meta-framework built on Vike + Vite - DI container, Eloquent-style ORM, Rudder CLI, middleware, queues, and auth.
- [Vike Lite](https://github.com/node-ecosystem/vike-lite#readme) - Lite version of Vike web framework
