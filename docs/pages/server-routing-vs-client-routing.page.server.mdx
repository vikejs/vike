...
import { ServerRouting, ClientRouting, DefaultRouting } from '../components'

Vike has first-class support for both *Server(-side) Routing* and *Client(-side) Routing*.
...
If you don't use a Vike extension <Link href="/vike-packages#ui-frameworks"><code>vike-react</code>, <code>vike-vue</code>, or <code>vike-solid</code></Link>, then we recommend to use Server Routing by default as it leads to a significantly simpler architecture.
...
Similarly to the previous section, when using <Link text="Nested Layouts" href="/layouts#nested-layouts" />, the state of the outer page is preserved, which means Nested Layouts cannot be implemented using Server Routing.
