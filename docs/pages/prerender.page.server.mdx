...
<Warning>
  `prerender()` is deprecated in favor of `onBeforePrerenderStart()`, see
  <ul>
...
The `prerender()` hook is executed during <Link text="pre-rendering" href="/pre-rendering" /> (and only during pre-rendering) and has two purposes:
