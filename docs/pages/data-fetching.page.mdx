...
data fetching is always exectued in Node.js.

But, if we use [Client Routing](/useClientRouter),
we can define `onBeforeRender()` in `.page.js` instead, in which case, `onBeforeRender()` is called in the browser instead of Node.js upon page navigation.
We usually want to do this if we want to minimize requests made to our Node.js server.

See [`export { onBeforeRender }` (`.page.js`)](/onBeforeRender-isomorphic).
