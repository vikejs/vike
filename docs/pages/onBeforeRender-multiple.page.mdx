import { Note, Link } from 'libframe-docs/components'

<Note type="error">Support for multiple `onBeforeRender()` hooks is deprecated. Use <Link href="data-fetching#global-logic" text="Custom Hooks"/> instead. Reach out on Discord or GitHub if you cannot replace your multiple `onBeforeRender()` hooks with one `onBeforeRender()` hook + Custom Hooks.</Note>
