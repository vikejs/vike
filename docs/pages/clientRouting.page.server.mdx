import { Link, RepoLink, ReadingRecommendation } from 'vikepress'

Environment: `Browser`.

By default, `vite-plugin-ssr` does <Link href="/server-routing" />.

We opt in to <Link text="Client Routing" href="/client-routing" /> by:
 1. Setting the option `clientRouting` to `true`.
 2. Adapting our <Link text={<>client-side <code>render()</code> hook</>} href="/render-client" />.

```js
// /renderer/_default.page.client.js
// Environment: Browser

// Tell vite-plugin-ssr to do Client Routing
