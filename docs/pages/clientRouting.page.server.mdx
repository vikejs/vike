...
We opt into <Link text="Client Routing" href="/client-routing" /> by modifying `/renderer/_default.client.js` to:
 ...
 2. Update and adapt the <Link text={<>client-side <code>render()</code> hook</>} href="/render-client" /> to
    conditionally hydrate the DOM.

> The <Link text={<><code>vike-*</code> packages</>} href="/vike-packages" /> are doing this for you. You can use
> [Bati](https://batijs.github.io/) to easily scaffold a project using `vite-plugin-ssr` and one of the `vike-*`
> packages. This uses the [V1 design](/migration/v1-design).
