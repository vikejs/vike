...
Use [Bati](https://batijs.dev) to scaffold a new Vike app.

You can skip exploring Bati's UI for now and directly run the following command instead:

<BatiCmd />

> This scaffolds a Vike app that uses <UiFrameworkVikeExtension name links /> which are the official React, Vue, and Solid integrations maintained by the Vike team.
>
> While we recommend using <UiFrameworkVikeExtension name /> you can also <Link href="/new#without-vike-react-vue-solid">manually integrate React/Vue/Solid yourself</Link>.
