...
Common causes:
 ...
 - **Browser cache**: Your browser's cache may load cached & outdated JavaScript. This may cause hydration mismatches if a component is outdated on the client-side and renders something else than the up-to-date component on the server-side. Clear your browser cache and try again.
    > In principle, this shouldn't happen because Vite always busts the cache whenever a file is changed. If you get issues with hydration mismatches related to browser caching, then let us know and we'll look into it.
