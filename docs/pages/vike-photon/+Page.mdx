...
import { PhotonDescription, PhotonBuiltInServer } from './components.ts'
...
<Warning>
  `vike-photon` is deprecated — it has been replaced by `+server`, see <Link href="/migration/server">Migration to `+server`</Link>.
</Warning>
...
Where:
 - `apply()` installs the middleware of Vike and Vike extensions (powered by [`universal-middleware`](https://universal-middleware.dev)).
   > Context populated by middlewares added by `apply()` is automatically merged into <Link href="/pageContext">`pageContext`</Link>.
...
export default {
  ...
  photon: { // [!code ++]
    ...
    // @docpress-uncomment compress: 'static' // [!code ++]
  } // [!code ++]
} satisfies Config
```
...
export default {
  ...
  photon: { // [!code ++]
    ...
    static: false, // [!code ++]
    // Settings // [!code ++]
    // @docpress-uncomment static: { maxAge: 3600, /* ... */ } // [!code ++]
  } // [!code ++]
} satisfies Config
```
