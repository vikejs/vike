...
<Warning>
  `vike-photon` is being replaced by a native implementation of universal deployment in Vike. See [Migration to `universal-deploy`](/migration/universal-deploy).
</Warning>

## 0. Native `universal-deploy`

It's recommended to use the new native implementation of universal deployment in Vike instead of `vike-photon`.
...
In production, Photon uses [`@vikejs/sirv`](https://universal-middleware.dev/middlewares/sirv) to serve static files.
