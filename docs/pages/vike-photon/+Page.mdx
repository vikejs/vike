import { Link, Warning, NoteWithoutIcon } from '@brillout/docpress'
...
The `vike-photon` extension <VikePhotonDescription />

This page contains the entire documentation for `vike-photon`.

> If you're using <Link href="/vike-server">`vike-server`</Link> then  <Link href="/migration/vike-photon">migrate to `vike-photon`</Link>.
...
#### Scaffold new `vike-photon` app

Use <Link href="/new">vike.dev/new</Link> for creating a new Vike + `vike-photon` app.

#### Add `vike-photon` to existing app
...
Optionally, if Photon's built-in server isn't sufficient for you (e.g. [file uploads](https://hono.dev/examples/file-upload) or <Link href="/api-routes">API routes</Link>), create your own server as shown below.

> Photon's built-in server supports only basic features such as SSR. This may or may not be sufficient depending on your needs.
...
<Tabs>
  <TabList>
    <Tab>Node/Bun/Deno</Tab>
    <Tab>Cloudflare</Tab>
    <Tab>Vercel</Tab>
  </TabList>
  <TabPanel>
    In production run `$ NODE_ENV=production node ./dist/server/index.mjs` (or Bun/Deno).
  </TabPanel>
  <TabPanel>
