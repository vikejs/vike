...
Where:
 - `apply()` installs the middleware of Vike and Vike extensions (powered by [Universal Middlewares](https://universal-middleware.dev)).
   > Context populated by middlewares is automatically merged into <Link href="/pageContext">`pageContext`</Link>.
