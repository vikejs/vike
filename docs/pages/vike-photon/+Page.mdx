...
You can disable it or only enable it for static files:
...
export default {
  ...
  photon: {// [!code ++]
    ...
    compress: false,// [!code ++]
    // Only static files will be compressed// [!code ++]
...
<Warning>
Using `getMiddlewares(...)` to configure compression has been deprecated.
Please remove its usage from your server entry file, and replace it with the configuration above.
...
In production, `vike-photon` uses [sirv](https://universal-middleware.dev/middlewares/sirv) to serve static files.
...
export default {
  extends: [vikePhoton],
  photon: {// [!code ++]
    // Do not serve static files// [!code ++]
    sirv: false// [!code ++],
    // See github.com/lukeed/sirv/tree/main/packages/sirv#api for advanced options// [!code ++]
    // @detype-uncomment sirv: { root: '/dist/client', maxAge: 3600 }// [!code ++]
  }// [!code ++]
} satisfies Config
```
