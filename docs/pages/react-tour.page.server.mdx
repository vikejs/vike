...
The `_default` files can be overridden. For example, we can overriding the `render()` hook for rendering some of our pages with a completely different UI framework such as Vue.
