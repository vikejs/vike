...
By using <Link href="/file-env">`.shared.js` (or `.client.js`)</Link> you tell Vike to also (or only) load and execute `data()` on the client-side.
...
By using `.client.js`,
you tell Vike to load and execute `data()` only on the client-side (never on the server-side).
...
>
> Note that, for pre-rendered pages, data fetched with the `data()` hook is frozen at build-time: the value returned by `data()` is saved as a static JSON file <Link href="/pageContext.json">`dist/client/some-page/index.pageContext.json`</Link> generated during pre-rendering.

### `.shared.js`

By using `.shared.js`,
you tell Vike to load and execute `data()` also on the client-side:
...
>
> That said, if you want to minimize requests made to your server, then it can make sense to run `data()` also on the client-side. See: <Link href="/pageContext.json#avoid-pagecontext-json-requests" doNotInferSectionTitle noBreadcrumb />.
