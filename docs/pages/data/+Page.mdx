...
>
> That said, if you want to minimize requests made to your server, then it can make sense to run `data()` on the client-side. In that case Vike won't make any requests to your server upon page navigation (otherwise Vike makes `index.pageContext.json` HTTP GET requests in order to pass the data fetched on the server-side to the client-side). If you define `onBeforeRender()`, then also make sure to change the environment of your `onBeforeRender()` render hook, see <Link href="/onBeforeRender#client-side" />.
...
> Alternatively to `.shared.js` and `.client.js`, you can change the <Link href="/meta#example-modify-data-env">`meta.env` setting</Link> of the `data()` hook in a global fashion, alleviating the need to add `.shared.js` / `.client.js` to each `+data.js` file.
