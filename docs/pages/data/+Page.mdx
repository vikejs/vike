...
By default (<Link href="#client-side">it's configurable</Link>) the `data()` hook always runs the server-side, which means you can directly use ORM/SQL database queries:
