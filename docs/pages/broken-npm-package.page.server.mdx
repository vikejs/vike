...
<Warning>While using `vite.config.js > ssr.noExternal` fixes the problem, we recommend to contact the npm package author. For example: [aws-amplify/amplify-ui#3155](https://github.com/aws-amplify/amplify-ui/issues/3155). Vite is fast because it transpiles only what is strictly needed. To preserve Vite's speed, library authors should publish valid code.</Warning>
