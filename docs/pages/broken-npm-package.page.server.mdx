...
The debug logs will help you know which React component is `undefined` (or an `object`). You'll likely discover that the React component is imported, while the import value is `undefined` (or an object instead of a React component function/class) because of some CJS/ESM interoperability quirks.
