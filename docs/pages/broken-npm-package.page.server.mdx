...
For React users we recommend to set `ssr.noExternal` to `true`.
...
export default {
  ssr: {
    // It's recommended to set ssr.noExternal to true for React users. (Because the server-side
    // code of React libraries often need to be transpiled and cannot run directly on Node.js).
    noExternal: true // Always transpile the server-side code of libraries.
  }
}
```

Since Next.js always transpiles the server-side of libraries, many library authors publish server-side code that is invalid JavaScript and always require to be transpiled. They cannot run directly on Node.js.

> You may be tempted to only add your React libraries to `ssr.noExternal` but that often isn't enough: when you add a React library to `ssr.noExternal` you may need to also add some of its dependencies (`ssr.noExternal` is shallow). That's why the easiest is to set `ssr.noExternal` to `true`.
