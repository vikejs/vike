...
> but every SSR Vue app actually uses `createSSRApp` and `h`.
> Nuxt abstracts this away from us and gives us a faster getting started but
> we loose control over a central piece of architecture which means that things will eventualy break as we scale.
> We'll then loose *much* more time circumventing Nuxt's black box & limitations.
