...
The `globalContext` is created when the server starts, and lives until the server process shuts down. For example, if your server process runs for five days, then the `globalContext` object lives for five days.
...
When a user starts visiting your website, a new `globalContext` object is created that lives until the user closes your website. If the user opens your website in multiple browser tabs, then there is one `globalContext` per tab.
