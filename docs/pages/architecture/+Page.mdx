...
import { JustAMiddleware, UiFrameworkVikeExtension, UiFrameworkVikeExtension } from '../../components'
...
The <UiFrameworkVikeExtension /> integrate UI frameworks. But, if you want, instead of using <UiFrameworkVikeExtension name /> you can use the hooks `onRenderHtml()` and `onRenderClient()` for complete control over how pages are rendered.
