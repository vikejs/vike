...
export default {
   Page,
   // Parameterized route matching URLs such as '/product/1337' and `/product/macbook'
   route: '/product/@id'
} satisfies Config
```
```js
...
The `+config.js` file of pages (`/pages/some-page/+config.js`) can override the default values defined by `/pages/+config.js`.
