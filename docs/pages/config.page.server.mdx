...
Deep dive into Vike's configuration.

> For the list of Vike's built-in configs, see <Link href="/Config" /> instead.
...
`/pages/+*.js` files (directly under `/pages`) allows you to define hooks and settings that apply to all (<Link href="#different-defaults">or only a group of</Link>) pages.
...
> `config.title` is provided by the <Link text="UI framework integration" href="/vike-packages#ui-frameworks" />, otherwise if you don't use a integration package then you can define `config.title` yourself (see <Link href="/meta" />).
...
The `renderer/` directy doesn't add any functionality: `/renderer/+config.h.js` is equivalent to `/pages/+config.h.js`. It's just an (optional) convenience for moving renderer files outside of `pages/` (to avoid cluttering the `pages/` directory). That way, the entire renderer logic can be organized and tucked into a single place `renderer/`.
