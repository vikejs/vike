...
import { HeaderFileIntro, VikeExtensionsUiFrameworks, VikeExtensionsUiFrameworksNames } from '../components'
...
> `config.title` is provided by the <VikeExtensionsUiFrameworks />. If you don't use <VikeExtensionsUiFrameworksNames /> then you can define `config.title` yourself, see <Link href="/meta" />.
...
> The `renderer/` directory is usually only used by users who don't use a <VikeExtensionsUiFrameworks />.
