...
import { HeaderFileIntro, VikeExtensionUiFramework, VikeExtensionUiFrameworkNames } from '../components'
...
> `config.title` is provided by the <VikeExtensionUiFramework />. If you don't use <VikeExtensionUiFrameworkNames /> then you can define `config.title` yourself, see <Link href="/meta" />.
...
> The `renderer/` directory is usually only used by users who don't use a <VikeExtensionUiFramework />.
