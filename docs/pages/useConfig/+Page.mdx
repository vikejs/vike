...
<ConfigSpec
  env="client, server"
  providedByExtension={{ kind: 'component-hook', name: 'useConfig()', noCustomGuide: true }}
  global={null}
/>
...
> Normally hooks can only be used inside UI components — `useConfig()` is different: it can also be used inside Vike hooks. We call it a *universal hook*. So far, `useConfig()` is the only universal hook.
...
> The `<Config>` and `<Head>` components are just wrappers around `useConfig()` — they're only for convenience.
> For example, the following achieves the same:
> ```tsx
...
> The `<meta name="description">` tag is only shown to bots. See the explanation in the <Link href="#html-streaming" /> section below.
...
This might seem like an issue, but it actually isn't — see the explanation in the <Link href="#bots"/> section below.
...
```bash
# What bots and crawlers get: no HTML Streaming, just "classic SSR"
$ curl http://localhost:3000/movies
# What human users get: HTML Streaming
$ curl http://localhost:3000/movies -N -H "User-Agent: chrome"
```

> Consequently, `<head>` tags intended for bots (SEO, PWA settings, ...) are included in the HTML. They might be missing for human users, but that isn't an issue.
...
Currently, `+title` is the only setting that uses this mechanism.
...
If you define settings multiple times, Vike chooses the first value in this order:

1. The value set by <Link href="#ui-components">`useConfig()` inside UI components</Link> (or <Link href="#config-head" />)
1. The value set by <Link href="#vike-hooks">`useConfig()` inside Vike hooks</Link>
1. The value set by the page's settings
1. The value set by the global settings
