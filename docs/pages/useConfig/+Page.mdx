...
> It's currently only implemented by `vike-react`. Contribution welcome [for `vike-vue`](https://github.com/vikejs/vike-vue/issues/175) and [for `vike-solid`](https://github.com/vikejs/vike-solid/issues/105).

> Both achieve the same:
> - `<Head><something /></Head>`
> - `<Config Head={<something />}/>`
> 
> There in't any `<Title>` component because it's expected to be a string: e.g. `<Title><span>hello</span></Title>` doesn't make sense.
