...
> It's what we call a *universal hook*: a hook that can be called inside UI components as well as inside Vike hooks. (So far `useConfig()` is the only universal hook.)
