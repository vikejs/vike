...
> These are Vike settings, see <Link href="/host" noBreadcrumb />/<Link href="/port" noBreadcrumb />/<Link href="/mode" noBreadcrumb />/<Link href="/prerender" noBreadcrumb />/<Link href="/force" noBreadcrumb />/<Link href="/root" noBreadcrumb />. Note that that Vike aliases some Vite settings for convenience, see <Link href="#vite-settings"/>.
...
> Each of these is a Vike setting that aliases a Vite setting (see <Link href="/host" noBreadcrumb />/<Link href="/mode" noBreadcrumb />/<Link href="/port" noBreadcrumb />/<Link href="/force" noBreadcrumb />). Vike also aliases Vite's [`root`](https://vite.dev/config/shared-options.html#root) setting over <Link href="/root" noBreadcrumb /> (Vite's CLI takes `root` as a positional argument instead of a flag). Each Vike CLI option sets a <Link href="/settings">Vike setting</Link>.
>
