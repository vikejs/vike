...
The <Link href="/extensions">Vike extension</Link> `vike-vue` is a full-fledged [Vue](https://vuejs.org) integration, with a high-level DX like a conventional framework such as Nuxt.

The documentation for using `vike-vue` can be found throughout this website (`vike.dev`).

> Guides, such as <Link href="/data-fetching" />, assume you're using Vike with <UiFrameworkExtension name noLink comma />. If you aren't then see the sections `Without vike-{react,vue,solid}` such as <Link href="/useData#without-vike-react-vue-solid" />.
...
- <Link noBreadcrumb href="/rootAttributes" />
