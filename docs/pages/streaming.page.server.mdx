...
import progressiveRenderingDemo from './streaming/progressive-rendering-demo.webm'
import { UseUiFrameworkVikeExtensionAnyHint } from '../components'

Vike not only has first-class support for HTML streaming (aka SSR streaming), but also provides you with extensive control over the stream.
