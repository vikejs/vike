...
## Built-in
 ...
 - **`pageContext.isHydration`**: *[only in the browser, and only when using `useClientRouter()` i.e. Client-side Routing]* boolean that is `true` when the page is already rendered to HTML. (When the page is already rendered to HTML we merely have to hydrate it. When doing SSR, it's `true` for the first page the user navigates to, and `false` when the user navigates to a new page.)
