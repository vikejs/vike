## Built-in
 ...
 - **`pageContext.urlParsed`**: `{ origin: null|string, pathname: string, search: Record<string,string>, hash: string }` (e.g. `{ origin: 'https://example.com', pathname: '/product/42', search: { details: 'yes' }, hash: 'reviews' }`). Also note the other properties `pathnameWithBaseUrl: string`, `hasBaseUrl: boolean`, `hashString: null|string`, and `searchString: null|string`. If we want the ordered list of all search parameters we can do: `new URLSearchParams(pageContext.urlParsed.searchString||'')`.
