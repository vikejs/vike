## Built-in
 ...
 - **`pageContext.urlPathname`**: the URL's pathname (without Base URL). (E.g. `/product/42` for `pageContext.url === 'https://example.com/some-base-url/product/42?details=yes#reviews'`).
 - **`pageContext.urlParsed`**: `{ origin: null | string, pathname: string, search: Record<string, string>, hash: string }`. (E.g. `{ origin: 'https://example.com', pathname: '/product/42', search: { details: 'yes' }, hash: 'reviews' }`.) Also note the other properties: `pathnameWithBaseUrl: string`, `hasBaseUrl: boolean` (almost always `true`), `hashString: null | string`, `searchString: null | string`. If we want the ordered list of all search parameters then we do: `new URLSearchParams(pageContext.urlParsed.searchString || '')`.
