## Built-in
 ...
 - **`pageContext.urlParsed`**: `{ origin: string, pathname: string, search: Record<string, string>, hash: string }`. (E.g. `{ origin: 'https://example.com', pathname: '/product/42', search: { details: 'yes' }, hash: 'reviews' }`.) Also note the other properties: `pathnameWithBaseUrl: string`, `hasBaseUrl: boolean` (is almost always `true`), `hashString: null | string`, `searchString: null | string`. If we want the ordered list of all search parameters then we do: `new URLSearchParams(pageContext.urlParsed.searchString || '')`.
