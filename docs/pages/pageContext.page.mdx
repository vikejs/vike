...
## Custom
 ...
 - The `pageContext` values we returned in our page's `onBeforeRender()` hook (if we defined one).
 - The `pageContext` values we returned in our `_default.page.server.js`'s `onBeforeRender()` hook (if we defined one).
