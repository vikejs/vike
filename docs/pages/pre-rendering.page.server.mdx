...
Pre-rendering means to render the HTML of all our pages at build-time (when we run `$ vite build`).

If we don't use pre-rendering, then the HTML of our pages is rendered at request-time (when the user navigates to that page).

When using pre-rendering, we don't need a Node.js server: our app consists only of static assets (HTML, JS, CSS, images, ...)
that we can deploy to a so-called "static host" such as <Link href="/github-pages" text="GitHub Pages" />, [Cloudflare Pages](/cloudflare-pages), or [Netlify](/netlify).
