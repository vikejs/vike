...
Upon rendering a page, `onBeforeRoute()` is the first hook that `vite-plugin-ssr` calls. This means that `vite-plugin-ssr` and the rest of our app doesn't have to deal with localized URLs anymore and can use `pageContext.locale` instead.
