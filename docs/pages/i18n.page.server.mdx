...
define a second hook called `onPrerenderStart()`:
...
We basically use `onBeforePrerenderStart()` to determine URLs and/or load data, and use `onPrerenderStart()` merely to
manipulate the URLs and set `pageContext.locale`.

> Note that `onPrerenderStart(prerenderContext)` is a global hook we can define only once, while
> `onBeforePrerenderStart()` (it doesn't have any argument) is a per-page hook we can define multiple times.
...
// we use onBeforePrerenderStart() to duplicate and localize URLs and their
// pageContext
export { onBeforePrerenderStart }
