...
Upon rendering a page, `onBeforeRoute()` is the first hook that `vite-plugin-ssr` calls, which means that the rest of our app doesn't have to deal with localized URLs anymore and can simply use `pageContext.locale` instead.
