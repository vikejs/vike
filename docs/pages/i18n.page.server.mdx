...
> Despite what the naming suggests, note that vite-plugin-ssr calls our `onBeforePrerender()` hook *after* our `prerender()` hooks.

Alternatively, if we need to load data that depends on localization, we don't define any `onBeforePrerender()` and instead only use `prerender()` to generate localized URLs as well as fetch data:

```js
// /pages/product.page.server.js

// In this example, we don't define any onBeforePrerender() and instead
// only use prerender() to duplicate/localize all URLs and their pageContext
export { prerender }
