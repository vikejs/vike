...
The `renderPage()` function renders your pages.

If you want to use server-side rendering (SSR), then you need to manually integrate `renderPage()` to your server (see <Link href="#usage" />).

If you <Link text="pre-render" href="/pre-rendering" /> all your pages, then you don't need to call `renderPage()` manually yourself, because:
 - Vite-plugin-ssr automatically integrates `renderPage()` into Vite's development server (`$ vite`/`$ vite dev`) and Vite's preview server (`$ vite preview`).
