...
The `renderPage()` function enables you to integrate and embed vike into any server. It's [optional](#optional) (it's only needed if you use SSR).
...
The `pageContext.httpResponse.body` value is the HTML string returned by the <Link text={<><code>render()</code> hook</>} href="/render-hook" /> with additional `<script>` and `<style>` tags automatically injected by Vike.
...
> When modifying your server, you may need to manually restart your server for your changes to take effect. See [#562](https://github.com/brillout/vike/issues/562).
...
If you use server-side rendering (SSR) and you don't pre-render all your pages, then you need a production server and you need to use `renderPage()` in order to embed vike into your server.
