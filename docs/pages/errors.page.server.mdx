...
While vike is careful about not removing relevant information, it may mistakenly do so. In that case [create a new GitHub issue](https://github.com/brillout/vike/issues/new?assignees=&labels=bug+%3Aboom%3A&projects=&template=bug_report.yml). Until a vike maintainer fixes the issue you can use the debug flag `DEBUG=vps:error`.
