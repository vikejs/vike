...
<ConfigSpec
  ...
  cumulative
  providedBy={<ProvidedBy kind="setting" name="+Layout" />}
/>
...
You currently cannot use the <Link href="/data">`+data` hook</Link> to fetch data only for layouts. Instead, load all data (including both the page and layout data) using a single `+data` hook.

Alternatively, you can use <Link href="/data-fetching#page-data-with-tools">tools to fetch data at the component level</Link>. You can then fetch data for the layout component independently of page data.

Support for cumulative `+data` hooks is a work in progress. You'll then be able to define a `+data` hook for your layout in addition to defining `+data` hooks for your pages, see:
