...
Vike frameworks use the same interface as regular Vike apps: the only difference is that some page files (e.g. <Link text={<><code>/renderer/+onRenderHtml.js</code></>} href="/default-page" />) live in `app/node_modules/framework/` instead of `app/`.
