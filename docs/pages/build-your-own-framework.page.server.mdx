...
Vike frameworks use the same interface as regular Vike apps: the only difference is that some page files (e.g. <Link text={<><code>/renderer/+onRenderHtml.js</code></>} href="/config#renderer" />) live in `app/node_modules/framework/` instead of `app/`.
...
In other words, the framework takes care of the UI framework integration (React, Vue, ...) so that all the framework's users have left to do is to define UI components and `/pages/**/+*.js` files.
