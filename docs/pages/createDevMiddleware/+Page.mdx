...
> You don't need `createDevMiddleware()` if you use <Link href="/server">`+server.js`</Link>. (You also don't need it if you <Link href="/pre-rendering">pre-render</Link> and don't use a server at all.)
