...
You configure your Vike app by creating files that start with `+`, such as `+config.js` and `+Page.js`.
...
Both achieve the same, it's just convenience as the latter allows you to skip creating the `+config.js` file.

> Expect of the `+config.js` file, each `+` file corresponds to a <Link href="/settings">Vike setting</Link> or a <Link href="/hooks">Vike hook</Link>.
