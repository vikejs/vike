...
We call a configuration *global* when it cannot be *local* — it always applies to all pages. For example, <Link href="/base-url">Base URL settings</Link> always affect your app globally.
...
In contrast, the <Link href="/title">`+title` setting</Link> can be local, as each page can have a different title.
