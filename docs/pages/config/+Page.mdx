...
For example with the <Link href="/Layout">`Layout`</Link> and <Link href="/ssr">`ssr`</Link> settings:
...
export default {
  // The <Layout> for all pages (as overridable default)
  Layout,
...
export default {
  // The <Layout> of the product page (overriding the default set by /pages/+config.js)
  Layout,
