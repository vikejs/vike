...
import { IconPerPage, IconGlobal } from '../../components'
...
// @docpress-uncomment export default /* ... */
```
```ts
...
```yaml
/pages/product/@id/+config.ts # [!code --]
/pages/product/@id/Page.tsx # [!code --]
/pages/product/@id/Layout.tsx # [!code --]
/pages/product/@id/+Page.tsx # [!code ++]
/pages/product/@id/+Layout.tsx # [!code ++]
```
...
> Except of `+config.js`, any `+` file corresponds to a Vike <Link href="/settings">setting</Link> or <Link href="/hooks">hook</Link>.
...
For <Link href="#cumulative">cumulative configurations</Link>, the `.clear.js` filename suffix resets the inheritance chain: configurations from parent directories aren't inherited.
...
> The `.clear.js` suffix applies only to cumulative configurations: non-cumulative configurations are already cleared.
...
For <Link href="#cumulative">cumulative configurations</Link>, the `.default.js` filename suffix makes a configuration a fallback: it's used only for directories that don't define the configuration themselves.
...
> The `.default.js` suffix applies only to cumulative configurations: non-cumulative configurations already behave as defaults.
...
- <IconGlobal/> **Global configuration** — always applies to all pages.

  For example, the <Link href="/base-url">Base URL setting</Link> is global: changing the Base URL affects your whole app (all your pages).

  A global config cannot be applied to only a subset of pages.

  > There isn't any <Link href="#inheritance">config inheritance</Link> for global configs:
  >
  > ```yaml
...
- <IconPerPage/> **Per-page configuration** — can be defined on a per-page basis, as well as globally.

  For example, <Link href="/title">`+title`</Link> is a per-page setting: each page *can* have a different title.
  ...
  > } satisfies Config
  > ```
  > See also: <Link href="#inheritance" />.

### Groups

You can use <Link href="/routing#groups">page groups</Link> to better organize configuration inheritance.
...
// @docpress-replace Layout_ Layout
const Layout_ = 'import:../layouts/LayoutDefault.jsx:default' // [!code ++]
...
> The content of a `.jsx` or `.vue` file is runtime code for the client/server (it doesn't configure your app) and that's why Vike resolves it as pointer import.
