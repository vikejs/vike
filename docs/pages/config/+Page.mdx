...
A configuration is either *global* or *local*.

A global configuration always applies to all pages. For example, <Link href="/base-url">Base URL settings</Link> affect your app globally.
...
In contrast, the <Link href="/title">`+title` setting</Link> is local, as each page can have a different title.
