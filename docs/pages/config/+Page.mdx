...
> To control inheritance for <Link href="#cumulative">cumulative configurations</Link>, see <Link href="#cumulative-inheritance-control">Cumulative inheritance control</Link>.
...
#### Cumulative inheritance control

You can control how cumulative configurations inherit from parent directories by using filename suffixes.

**Default suffix (`.default`)**

The `.default` suffix makes a configuration act as a fallback that is only used when no regular (non-default) configurations are defined.

```yaml
# Fallback value
pages/+Layout.default.jsx
# Regular value - this overrides the default
pages/product/+Layout.jsx
```

If any regular configuration exists in the inheritance chain, all default configurations are ignored:

```yaml
# This default is ignored because a regular config exists below
pages/+Layout.default.jsx
# This regular config is used
pages/product/+Layout.jsx
```

**Clear suffix (`.clear`)**

The `.clear` suffix resets the inheritance chain, preventing configurations from parent directories from being included.

```yaml
# This layout won't be included
pages/+Layout.jsx
# This layout clears inheritance and starts fresh
pages/admin/+Layout.clear.jsx
# This layout will be added to the admin layout (not the root layout)
