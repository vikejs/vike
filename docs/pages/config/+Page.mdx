...
import { HeaderFileIntro, UiFrameworkVikeExtension, UiFrameworkVikeExtensionNames } from '../../components'
...
> `config.title` is provided by the <UiFrameworkVikeExtension />. If you don't use <UiFrameworkVikeExtensionNames /> then you can define `config.title` yourself, see <Link href="/meta" />.
...
> The `renderer/` directory is usually only used by users who don't use a <UiFrameworkVikeExtension />.
