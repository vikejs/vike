...
VPS (vite-plugin-ssr) differs from Next.js in two fundamental ways:
 ...
 1. VPS is server and deployment agnostic.
    > With VPS, you keep full control over your server and can use any deployment strategy.

Beyond these structural differences,
VPS also introduces features such as blazing fast HMR (powered by Vite), first-class SPA support (especially with the upcoming new VPS design), Domain-driven File Structue, state-of-the-art Code Splitting (powered by Vite/Rollup), Automatic Deployment Synchronization, Build Your Own Framework, etc.
...
With VPS, you integrate React yourself. This means you need to write a little bit more boilerplate but, in exchange, you get a lot more flexibility. This is why you can use tools like Relay or Apollo in ways that aren't possible with Next.js.
...
>
> Many users, including sponsors, leverage VPS's flexibility to deeply integrate Relay with SSR Streaming (which isn't possible with Next.js).
...
Except of React and server integration, VPS includes everything you'd expect from a frontend framework: blazing fast Client-side Routing, HMR (powered by Vite), Filesystem Routing, state-of-the-art Code Splitting (powered by Rollup), etc.


## Server/deployment agnostic

From a server persepctive, <Link text="VPS is just a middleware" href="/deploy" /> that runs anywhere (AWS, Vercel, Cloudflare, Deno Deploy, ...).

To integrate VPS, you just add VPS's middleware to your server (Express.js, Fastify, Edge Worker, Serverless Function,  ...).

With VPS, you have full control over your server/deployment, one of the most critical aspect of your architecture.
...
Last but not least, manual integration is fun, provides freedom, and is insightful. (Whereas fighting a framework is pesky.)
