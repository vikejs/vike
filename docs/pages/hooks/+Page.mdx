...
> - `Server-only (default)` if you use `+data.js` or `+onBeforeRender.js` which, by default, run only on the server-side.
> - `Client-only` if you use <Link href="/data#environment">`+data.client.js`</Link> or <Link href="/onBeforeRender#environment">`+onBeforeRender.client.js`</Link> (which run only on the client-side).
> - `Server & client` if you use `+data.shared.js` or `+onBeforeRender.shared.js` (which run on both the server- and client-side).
