...
export default pageContext => {
  ...
  if (!isUserAuthenticated) {
    return {
      // We use a high precedence to override all other routes.
      // This means unauthenticated users will always see the Login Page `login.page.js`
      precedence: 999
    }
  } else {
    // If the user is authenticated then do not show the Login Page.
    // E.g. if the user goes to `/admin`, then show the Admin Page.
    return false
  }
}
```
...
After the user successfully logs in, we can reload the page with `window.location.reload()` (<Link href="/server-routing"  noBreadcrumb={true} />) or <Link text={<code>navigte(window.location.pathname)</code>} href="/navigate" /> (<Link href="/client-routing" noBreadcrumb={true} />).

This approach preserves the URL during the entire login flow:
 1. Unauthenticated user goes to URL `/admin` and see the Login Page. (URL is `/admin`.)
 2. The users fills the sign in form and successfully logs in. (URL is still `/admin`.)
 3. The page is reloaded and the user now sees the Admin Page. (URL is still `/admin`.)
...
We can also perform a page redirection instead of using this approach. See sections below.
