...
if a user is not authenticated and goes to `/admin/panel` then we redirect the user to `/login`.

Alternatively, we can use a Route Function instead of doing a URL redirection.
...
After the user has successfully completed the Login Form rendered at `/admin/panel`, we can do a `window.location.reload()` and `/admin/panel` will now render the Admin Panel. This means that the user always stays on `/admin/panel` during the entire authentication flow.
