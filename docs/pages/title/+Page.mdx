...
On the client-side, upon <Link href="/client-routing">client-side page navigation</Link>, the `title` setting dynamically updates the page's title by mutating the DOM with [`document.title = 'Some Title'`](https://developer.mozilla.org/en-US/docs/Web/API/Document/title). It also updates the `<meta property="og:title">` tag.
