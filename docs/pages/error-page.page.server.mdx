...
The page `_error.page.js` is rendered when:
 ...
 3. We execute <Link href="/RenderErrorPage" text={<code>throw RenderErrorPage()</code>}></Link>.

For `1.` and `3.`, `vite-plugin-ssr` automatically sets the following:
 - `pageContext.is404 === true`
 - `pageContext.pageProps.is404 === true`
    > We can `throw RenderErrorPage({ pageContext: { is404: false }})` to set `pageContext.is404 === false` and `pageContext.pageProps.is404 === false` instead.
...
That way, we can use `pageContext.pageProps.is404` to decide what error message to show the user.
