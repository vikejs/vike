...
export async function data(pageContext) {
  // Commonly used pageContext properties in data()
...
The `data()` hook is usually used togther with the component hook <Link href="/useData">`useData()`</Link> which is provided by the <UiFrameworkVikeExtension />.
...
In general, for improved DX, we recommend always using `data()` together with a `useData()` implementation. That said, you can also use `data()` without `useData()`:
