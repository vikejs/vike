...
You can set initial `pageContext` values, such as information about the authenticated user, at <Link text={<code>renderPage(pageContextInit)</code>} href="/renderPage" />.
...
Keep in mind that you'll then have to ensure that your `data()` hook is isomorphic: it needs to be able to run on the server-side as well as on the client-side.
