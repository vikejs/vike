...
- <Link href='/examples/vue-full/pages/star-wars/index/+data.ts' />
...
> `pageContext.user` is a custom property, see:
...
If an error is thrown by `data()`, then Vike renders your <Link text="error page" href="/error-page" /> and there is usually nothing for you to do (beyond defining `/pages/_error/+Page.js`).
...
async function data(pageContext) {
  ...
  if (response.status === 404) {
    // Tell the user exactly what wrong
...
This means that, beyond using <Link href="/useData">`useData()`</Link>, you can also access the data in other hooks over `pageContext.data`, see <Link href="/pageContext" />.
...
In general, for improved DX, we recommend to always use `data()` together with a `useData()` implementation. That said, you can also use `data()` without `useData()`:
