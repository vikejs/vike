...
In general, we recommend always using Vike extensions while falling back to a custom integration only if you have a clear reason.
...
> You can also eject a Vike extension, for progressively moving from the ejected implementation of the Vike extension towards a custom integration. See <Link href="/optional-control" />.

Large projects often need bespoke integrations, and opting out of Vike extensions can be a sensible choice. If that's your case, then chances are you already have a clear reason for wanting a custom integration instead of using a Vike extension.
