...
> **What is the Base URL?** Changing the Base URL (aka Public Base Path) is about changing the URL root of your app.
> For example, instead of deploying your app at `https://example.org/*` (i.e. `base: '/'`), you can set `base: '/some/nested/path/'` and deploy your app at `https://example.org/some/nested/path/*`.
...
3. Use the <ImportMeta prop='env.BASE_URL' /> value [injected by Vite](https://vitejs.dev/guide/build.html#public-base-path) to construct a `<Link>` component that prepends the Base URL. (Example `<Link>` implementation: <RepoLink path='/examples/base-url/pages/_components/Link.jsx' />.)
...
Example:
 - <RepoLink path='/examples/base-url/pages/_components/Link.jsx' /> (see the <code>&lt;Link&gt;</code> component built on top of <ImportMeta prop='env.BASE_URL' />)
