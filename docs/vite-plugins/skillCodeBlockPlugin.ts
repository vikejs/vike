...
// Injects the content of skills/vike/SKILL.md into the code block of pages/ai/+Page.mdx — before the MDX compiler runs, so that it's syntax highlighted like any other code block.
function skillCodeBlockPlugin(): Plugin {
  ...
  return {
    name: 'vike-docs:skill-code-block',
    enforce: 'pre',
    transform(code, id) {
